...
- A deleted RateLoop workspace revokes its OAuth token family. The next RateLoop Workspace invocation should make the host observe the protected server's authorization challenge and, when required, present one fresh OAuth action. Do not ask the user to remove unrelated plugins or reuse the deleted workspace.
