...
SUBMISSION_BOUNTY_AMOUNT="1000000" # 100 max voters * 10_000 participant unit
...
MIN_SUBMISSION_BOUNTY_AMOUNT=$((SUBMISSION_ROUND_MAX_VOTERS * 10000))
...
for ((i = 0; i < TOTAL_ITEMS; i++)); do
  ...
  cast send "$TOKEN" "approve(address,uint256)" "$QUESTION_REWARD_POOL_ESCROW" "$BOUNTY_AMOUNT" --private-key "$KEY" --rpc-url "$RPC" > /dev/null
	  ...
	  REVEAL_COMMITMENT=$(node "$SCRIPT_DIR/../scripts-js/buildSubmissionReservation.js" \
	    ...
	    "0" "$BOUNTY_AMOUNT" "$SUBMISSION_BOUNTY_REQUIRED_VOTERS" "$SUBMISSION_BOUNTY_ELIGIBILITY" \
  ...
  sleep 1
	  ...
	  cast send "$REGISTRY" "submitQuestionWithRewardAndRoundConfig(string,string[],string,string,string,uint256,(string,bytes32),bytes32,(uint8,uint256,uint256,uint8),(uint32,uint32,uint16,uint16),(bytes32,bytes32),(bool,uint8,uint64,uint8))" \
...
BUNDLE_BUILD_OUTPUT=$(node "$SCRIPT_DIR/../scripts-js/buildQuestionBundleReservation.js" \
  ...
  "$SUBMISSION_BOUNTY_REQUIRED_VOTERS" "$SUBMISSION_BOUNTY_ELIGIBILITY" \
