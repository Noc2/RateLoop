...
2. **Before mainnet** — L-Identity-7 (2-line deploy script pause-bracket for `setQuestionRewardPoolEscrow`). N-2 (override `_validateDescriptionForProposer` on `RateLoopGovernor`). N-3 (3-line `max(oldUnlock, newUnlock)` in `lockForGovernance`).
...
- N-2: `governance/RateLoopGovernor.sol:264-285`.
