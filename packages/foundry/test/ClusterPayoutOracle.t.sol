...
contract ClusterPayoutOracleTest is Test {
    ...
    function test_CorrelationEpochKeepsProposalWindowWhenConfigShrinks() public {
        oracle.proposeCorrelationEpoch(
            1,
            1,
            20,
        ...
        assertEq(snapshot.challengeWindowAtProposal, 2 hours);

        oracle.setOracleConfig(1, CHALLENGE_BOND, address(this));
        vm.warp(snapshot.proposedAt + 1 hours);

        vm.expectRevert(ClusterPayoutOracle.SnapshotNotFinalizable.selector);
        oracle.finalizeCorrelationEpoch(1);

        _challengeCorrelationEpoch(1, keccak256("still-open"));
    }

    function test_CorrelationEpochKeepsProposalWindowWhenConfigExpands() public {
        oracle.proposeCorrelationEpoch(
            1,
            1,
            20,
        ...
        oracle.setOracleConfig(3 hours, CHALLENGE_BOND, address(this));
        vm.warp(snapshot.proposedAt + 2 hours + 1);

        oracle.finalizeCorrelationEpoch(1);
    }

    function test_RoundPayoutSnapshotKeepsProposalWindowWhenConfigShrinks() public {
        oracle.proposeCorrelationEpoch(
            1,
            1,
            20,
        ...
        ClusterPayoutOracle.RoundPayoutProposal memory proposal = oracle.roundPayoutProposal(snapshotKey);
        assertEq(proposal.challengeWindowAtProposal, 2 hours);

        oracle.setOracleConfig(1, CHALLENGE_BOND, address(this));
        vm.warp(proposal.proposedAt + 1 hours);

        vm.expectRevert(ClusterPayoutOracle.SnapshotNotFinalizable.selector);
        oracle.finalizeRoundPayoutSnapshot(snapshotKey);

        _challengeRoundPayoutSnapshot(snapshotKey, keccak256("still-open"));
    }

    function test_RoundPayoutSnapshotKeepsProposalWindowWhenConfigExpands() public {
        oracle.proposeCorrelationEpoch(
            1,
            1,
            20,
        ...
        oracle.setOracleConfig(3 hours, CHALLENGE_BOND, address(this));
        vm.warp(proposal.proposedAt + 2 hours + 1);
    ...
    function test_FinalizedRoundPayoutSnapshotReproposalWithLiveParentRevertsSnapshotExistsWhenUnconsumed() public {
        ...
        ClusterPayoutOracle.RoundPayoutProposal memory original = oracle.roundPayoutProposal(snapshotKey);
        assertEq(uint8(original.snapshot.status), uint8(IClusterPayoutOracle.SnapshotStatus.Finalized));
        assertEq(original.snapshot.weightRoot, keccak256("leaf"));
    }
