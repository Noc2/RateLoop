...
contract QuestionRewardPoolEscrowTest is VotingTestBase {
    ...
    function testVeryHighValueRewardPoolRequiresEconomicParticipantFloor() public {
        RoundLib.RoundConfig memory roundConfig = RoundLib.RoundConfig({
            epochDuration: uint32(EPOCH_DURATION), maxDuration: uint32(EPOCH_DURATION), minVoters: 8, maxVoters: 8
        });
    ...
    function testSyncedBountyWindowQualifiesPreCloseCommitsRevealedAfterClose() public {
        RoundLib.RoundConfig memory roundConfig = RoundLib.RoundConfig({
            epochDuration: uint32(EPOCH_DURATION), maxDuration: uint32(EPOCH_DURATION), minVoters: 3, maxVoters: 3
        });
    ...
    function testCreateRewardPoolRejectsRequiredVotersAboveSettlementVoters() public {
        RoundLib.RoundConfig memory roundConfig = RoundLib.RoundConfig({
            epochDuration: uint32(EPOCH_DURATION), maxDuration: uint32(EPOCH_DURATION), minVoters: 3, maxVoters: 5
        });
    ...
    function testGovernanceCanRepointUnqualifiedQuestionBundleClusterOracle() public {
        ...
        bytes32 root = replacementOracle.payoutWeightLeaf(payoutWeight);
        _finalizeBundleClusterPayoutSnapshotWithRoot(
            replacementOracle, bundleId, 0, 3, 30_000, payoutWeight.effectiveWeight, root
        );
        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
    ...
    function testBundleRewardWaitsForFinalizationVetoWindowBeforeQualificationAndClaim() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        (uint256 firstRoundId, uint256 secondRoundId) =
            _settleBundleRoundSetWithoutBundleSync(contentIds, _threeVoters(), _directions(true, true, false));
        _recordBundleRoundSetTerminals(contentIds, firstRoundId, secondRoundId);
        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
        ...
        _finalizeBundleClusterPayoutSnapshotWithRootNoVetoWait(
            oracle, bundleId, 0, 3, 30_000, payoutWeight.effectiveWeight, oracle.payoutWeightLeaf(payoutWeight)
        );
        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
        assertEq(
            rewardPoolEscrow.claimableQuestionBundleRewardWithPayoutWeight(
        ...
        _warpPastBundleRewardSnapshotVeto(oracle, bundleId, 0);
        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
        vm.prank(voter1);
        assertGt(rewardPoolEscrow.claimQuestionBundleReward(bundleId, 0, payoutWeight, new bytes32[](0)), 0);
    }
    ...
    function testReplacementQuestionBundleOracleCannotPreSquatBeforeRepointAndCanProposeAfterPin() public {
        ...
        replacementOracle.proposeCorrelationEpoch(
            ...
            1,
            1,
            keccak256(abi.encode("bundle-cluster-root", bundleId, uint256(0))),
            keccak256("params"),
            keccak256("epoch-artifact"),
            "ipfs://bundle-epoch",
            _bundleEpochSources(bundleId, 1)
        );

        assertEq(rewardPoolEscrow.roundPayoutSnapshotSourceReadyAt(4, bundleId, bundleId, 1), 0);

        vm.prank(owner);
        rewardPoolEscrow.repointQuestionBundleClusterPayoutOracle(bundleId, address(replacementOracle));

        vm.prank(address(replacementOracle));
        assertGt(rewardPoolEscrow.roundPayoutSnapshotSourceReadyAt(4, bundleId, bundleId, 1), 0);
    ...
    function testGovernanceCannotRepointQuestionBundleClusterOracleWithExistingSnapshot() public {
        _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        vm.mockCall(
            ...
            abi.encodeWithSelector(
                rewardPoolEscrow.roundPayoutSnapshotSourceReadyAt.selector, uint8(4), bundleId, bundleId, uint256(1)
            ),
        ...
        vm.prank(owner);
        vm.expectRevert("Oracle snapshot exists");
        rewardPoolEscrow.repointQuestionBundleClusterPayoutOracle(bundleId, address(replacementOracle));
    }
    ...
    function testGovernanceCannotRepointQuestionBundleClusterOracleToWrongConsumer() public {
        ...
        replacementOracle.setOracleConfig(1 hours, 5e6, address(this));
        replacementOracle.setRoundPayoutSnapshotConsumer(
            replacementOracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), address(this)
        );
    ...
    function testGovernanceCannotRepointQualifiedQuestionBundleClusterOracle() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        vm.prank(owner);
        vm.expectRevert("Bundle already consumed");
        rewardPoolEscrow.repointQuestionBundleClusterPayoutOracle(bundleId, address(replacementOracle));
    }

    // Legacy recovery bodies below model pre-finality deployments where a qualified snapshot
    // could still be rejected. Fresh deployments wait out the veto window before qualification.
    function legacyRecoveredBundleSnapshotRoundSetCanReopenAndClaim() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyRecoveredBundleSnapshotRoundSetCanRepointToReplacementOracleAndClaim() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        vm.prank(address(replacementOracle));
        uint64 replacementSourceReadyAt = rewardPoolEscrow.roundPayoutSnapshotSourceReadyAt(4, bundleId, bundleId, 1);
        assertGt(replacementSourceReadyAt, 0);
        if (block.timestamp < replacementSourceReadyAt) vm.warp(replacementSourceReadyAt);
        _finalizeBundleClusterPayoutSnapshotWithRoot(
    ...
    function legacyRecoveredBundleSnapshotRoundSetCanReopenAndRefundWithoutClaim() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("replacement-bundle-refund-snapshot");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _finalizeBundleRoundPayoutSnapshotWithRoot(
            oracle, bundleId, 0, uint64(1), 3, 30_000, replacementWeight.effectiveWeight, replacementRoot
        );
    ...
    function legacyRecoveredBundleSnapshotRoundSetCanRefundAfterGraceWithoutReplacement() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyRecoveredBundleRefundRevertsWhenReplacementFinalizedBeforeReopen() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
    ...
    function testRecoveredBundleRefundRevertsWhenReplacementProposedBeforeReopen() public {
        (ClusterPayoutOracle oracle,, uint256 bundleId, IClusterPayoutOracle.PayoutWeight memory payoutWeight) =
            _recoverBundleRoundSetForReplacementRefundRace();

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("replacement-bundle-proposed-before-refund");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _proposeBundleRoundPayoutSnapshotWithRoot(
            oracle, bundleId, 0, uint64(1), 3, 30_000, replacementWeight.effectiveWeight, replacementRoot
        );
    ...
    function testRecoveredBundleRefundRevertsWhenReplacementChallengedBeforeReopen() public {
        (ClusterPayoutOracle oracle,, uint256 bundleId, IClusterPayoutOracle.PayoutWeight memory payoutWeight) =
            _recoverBundleRoundSetForReplacementRefundRace();

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("replacement-bundle-challenged-before-refund");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        bytes32 replacementSnapshotKey = _proposeBundleRoundPayoutSnapshotWithRoot(
            oracle, bundleId, 0, uint64(1), 3, 30_000, replacementWeight.effectiveWeight, replacementRoot
        );
        _challengeRoundPayoutSnapshot(oracle, replacementSnapshotKey, keccak256("challenged-replacement"));
    ...
    function testRecoveredBundleRefundRevertsWhenReplacementFinalizedInsideVetoBeforeReopen() public {
        (ClusterPayoutOracle oracle,, uint256 bundleId, IClusterPayoutOracle.PayoutWeight memory payoutWeight) =
            _recoverBundleRoundSetForReplacementRefundRace();

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("replacement-bundle-in-veto-before-refund");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        bytes32 replacementSnapshotKey = _proposeBundleRoundPayoutSnapshotWithRoot(
            oracle, bundleId, 0, uint64(1), 3, 30_000, replacementWeight.effectiveWeight, replacementRoot
        );
        uint64 proposedAt =
            oracle.roundPayoutSnapshotProposedAt(oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), bundleId, bundleId, 1);
        vm.warp(uint256(proposedAt) + uint256(oracle.challengeWindow()) + 1);
        oracle.finalizeRoundPayoutSnapshot(replacementSnapshotKey);
    ...
    function _recoverBundleRoundSetForReplacementRefundRace()
        internal
        returns (
        ...
        )
    {
        ...
        TestClusterPayoutOracle originalOracle = TestClusterPayoutOracle(protocolConfig.clusterPayoutOracle());
        contentIds = _submitBundleQuestions();
        bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        payoutWeight = _bundlePayoutWeight(bundleId, 0, contentIds[0], firstRoundId, 0);
        ...
        bytes32 snapshotKey = originalOracle.setFinalizedRoundPayoutSnapshotWithRoot(
            4,
            bundleId,
            bundleId,
            1,
            ...
            30_000,
            payoutWeight.effectiveWeight,
            keccak256(abi.encode("bundle-original-refund-race", bundleId))
        );

        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
        bytes32 rejectedDigest = originalOracle.roundPayoutSnapshotProposalDigest(snapshotKey);
        originalOracle.setRejectedRoundPayoutSnapshotDigest(snapshotKey, rejectedDigest, true);
        rewardPoolEscrow.recoverOrReopenSnapshotBundleRoundSet(bundleId, 0);
        ...
        vm.prank(owner);
        rewardPoolEscrow.repointQuestionBundleClusterPayoutOracle(bundleId, address(oracle));
        _finalizeBundleCorrelationEpoch(oracle, bundleId, 0);
    ...
    function testRecoveredBundleSnapshotRoundSetRejectsCurrentEligibilityDrift() public {
        _ensureTestClusterPayoutOracle(votingEngine);
        ...
        bytes32 snapshotKey = oracle.setFinalizedRoundPayoutSnapshotWithRoot(
            4, bundleId, bundleId, 1, 3, 30_000, 30_000, keccak256("bundle-root-before-drift")
        );
        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
        ...
        oracle.setFinalizedRoundPayoutSnapshotWithRoot(
            4, bundleId, bundleId, 1, 3, 30_001, 30_001, keccak256("bundle-root-after-drift")
        );
    ...
    function testRewardPoolQualificationChecksCommitHolderAddressBan() public {
        ...
        uint256 roundId =
            _settleRoundWith(_voters(delegate1, voter2, voter3), contentId, _directions(true, true, false));
        raterIdentityRegistry.setBanned(raterIdentityRegistry.addressIdentityKey(voter1), true);
    ...
    function testBundleClaimUsesRoundSpecificRaterIdentitiesAfterMigration() public {
        ...
        (uint256 firstRoundId, uint256 secondRoundId) = _settleBundleRoundSetWith(contentIds, voters, directions);
        ...
        assertEq(votingEngine.roundRaterRegistrySnapshot(contentIds[1], secondRoundId), address(raterIdentityRegistry));
    ...
    function testBundleClaimChecksCurrentRaterRegistryBanAfterMigration() public {
        ...
        _settleBundleRoundSetWith(contentIds, voters, directions);
    ...
    function testBundleFrontendFeeRequiresEveryQuestionRoundRecipient() public {
        address frontend2 = address(0xFEE2);
        _registerFrontend(frontend1);
        uint256[] memory contentIds = new uint256[](2);
        contentIds[0] = _submitQuestionWithContext("https://example.com/frontend-bundle-a", "frontend-bundle-a");
        ...
        secondRoundFrontendRegistry.setFrontend(frontend2, address(0xFEE3), true, false);
        vm.prank(owner);
        protocolConfig.setFrontendRegistry(address(secondRoundFrontendRegistry));
        contentIds[1] = _submitQuestionWithContext("https://example.com/frontend-bundle-b", "frontend-bundle-b");

        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);

        address[] memory voters = _threeVoters();
        bool[] memory directions = _directions(true, true, false);
        CommittedRound memory first = _commitBundleQuestionRoundWithFrontend(
            contentIds[0], voters, directions, keccak256("frontend-recipient-a"), frontend1
        );

        CommittedRound memory second = _commitBundleQuestionRoundWithFrontend(
            contentIds[1], voters, directions, keccak256("frontend-recipient-b"), frontend2
        );
        uint256 firstRevealable = _lastCommitRevealableAfter(votingEngine, first.contentId, first.roundId);
        uint256 secondRevealable = _lastCommitRevealableAfter(votingEngine, second.contentId, second.roundId);
        _warpPastTlockRevealTime(firstRevealable > secondRevealable ? firstRevealable : secondRevealable);
        _revealAndSettleCommittedRound(first, directions, true);
        _revealAndSettleCommittedRound(second, directions, true);

        uint256 frontend1BalanceBefore = usdc.balanceOf(frontend1);
        uint256 frontend2BalanceBefore = usdc.balanceOf(frontend2);
        ...
        assertEq(usdc.balanceOf(frontend1), frontend1BalanceBefore);
        assertEq(usdc.balanceOf(frontend2), frontend2BalanceBefore);
    }

    function testBundleFourVoterThresholdQualifiesWhenCompletersRevealBeforeClose() public {
        RoundLib.RoundConfig memory roundConfig = RoundLib.RoundConfig({
            epochDuration: uint32(EPOCH_DURATION), maxDuration: uint32(EPOCH_DURATION), minVoters: 4, maxVoters: 4
        });
        uint256[] memory contentIds = new uint256[](2);
    ...
    function testBundleRoundSetCountsCompleterAfterRaterIdentityRemintWithSameNullifier() public {
        ...
        CommittedRound memory first =
            _commitBundleQuestionRound(contentIds[0], voters, directions, keccak256("remint-same-nullifier-a"));
        raterIdentityRegistry.revokeHumanCredential(voter2);
        raterIdentityRegistry.resetNullifier(nullifier);
        raterIdentityRegistry.mint(voter2, nullifier);
        CommittedRound memory second =
            _commitBundleQuestionRound(contentIds[1], voters, directions, keccak256("remint-same-nullifier-b"));
        _revealAndSettleCommittedBundleRoundSet(first, directions, second, directions, true);
    ...
    function testBundleDelegatedFunderStaysExcludedAfterDelegateRotationAndRaterIdentityMigration() public {
        ...
        assertGt(rewardPoolEscrow.claimableQuestionBundleReward(bundleId, 0, newDelegate), 0);
    ...
    function testSubmissionBundleFundedAtMaxCompletersQualifies() public {
        ...
        _settleBundleRoundSetWith(contentIds, overfullVoters, overfullDirections);
    ...
    function testBundleRoundSetCountsDelegateCommitAfterDelegateRemoval() public {
        ...
        CommittedRound memory first =
            _commitBundleQuestionRound(contentIds[0], firstQuestionVoters, directions, keccak256("delegate-removed-a"));
        CommittedRound memory second =
            _commitBundleQuestionRound(contentIds[1], firstQuestionVoters, directions, keccak256("delegate-removed-b"));
        vm.prank(voter2);
        raterIdentityRegistry.removeDelegate();
        _revealAndSettleCommittedBundleRoundSet(first, directions, second, directions, true);
    ...
    function testBundleRewardRejectsDelegateStitchingDifferentHolders() public {
        ...
        vm.prank(voter2);
        raterIdentityRegistry.setDelegate(delegate1);
        CommittedRound memory first = _commitBundleQuestionRound(
            contentIds[0], _voters(delegate1, voter1, voter4), _directions(true, true, false), keccak256("stitch-a")
        );
        ...
        bool[] memory secondDirections = _directions(true, true, true, false);
        CommittedRound memory second =
            _commitBundleQuestionRound(contentIds[1], secondQuestionVoters, secondDirections, keccak256("stitch-b"));

        uint256 firstRevealable = _lastCommitRevealableAfter(votingEngine, first.contentId, first.roundId);
        uint256 secondRevealable = _lastCommitRevealableAfter(votingEngine, second.contentId, second.roundId);
        _warpPastTlockRevealTime(firstRevealable > secondRevealable ? firstRevealable : secondRevealable);
        _revealAndSettleCommittedRound(first, _directions(true, true, false), true);
        _revealAndSettleCommittedRound(second, secondDirections, true);
    ...
    function testBundleRewardRejectsOriginalDelegateAddressBanWhenNewDelegateClaims() public {
        address newDelegate = address(0xD1E);
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        vm.prank(owner);
    ...
    function testBundleRewardRejectsCommitHolderAddressBanWhenNewDelegateClaims() public {
        address newDelegate = address(0xD1E);
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        vm.prank(owner);
    ...
    function testDelegatedFunderStaysExcludedAfterDelegateRotationAndRaterIdentityMigration() public {
        ...
        assertGt(rewardPoolEscrow.claimableQuestionReward(rewardPoolId, roundId, newDelegate), 0);
        vm.prank(newDelegate);
        rewardPoolEscrow.claimQuestionReward(rewardPoolId, roundId);
    ...
    function testSetClusterPayoutOracleRejectsQuestionRewardConsumerMismatch() public {
    ...
    function testSetClusterPayoutOracleRejectsQuestionBundleConsumerMismatch() public {
        ClusterPayoutOracle oracle = _newEligibleClusterPayoutOracle();
        oracle.setOracleConfig(1 hours, 5e6, address(this));
    ...
    function testClusterRewardPoolCanSkipFinalizedEmptySnapshotAfterVetoWindow() public {
        ...
        vm.expectRevert("Cluster snapshot pending");
        rewardPoolEscrow.qualifyRound(rewardPoolId, roundId);
    ...
    function testArbiterCanReplacePreQualificationFinalizedClusterSnapshotMismatch() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        ...
        rewardPoolEscrow.qualifyRound(rewardPoolId, roundId);
        // Qualification alone does not flip the consumed flag. Claims still wait for a finalized,
        // post-veto oracle snapshot before moving funds.
        assertFalse(rewardPoolEscrow.isRoundPayoutSnapshotConsumed(1, rewardPoolId, contentId, roundId));

        // Claim one reward against the corrected snapshot. After the first paid wei, the consumer
        // reports consumed; post-veto finality prevents later rejections.
    ...
    function testClusterRewardPoolWaitsForFinalizationVetoWindowBeforeQualificationAndClaim() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        uint256 roundId = _settleRoundWith(_threeVoters(), contentId, _directions(true, true, false));
        ...
        rewardPoolEscrow.qualifyRound(rewardPoolId, roundId);
        vm.prank(voter1);
        assertGt(rewardPoolEscrow.claimQuestionReward(rewardPoolId, roundId, payoutWeight, new bytes32[](0)), 0);
    }

    function testPreQualificationRejectedClusterSnapshotRoundCanBeSkippedAndRefunded() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 expiresAt = block.timestamp + EPOCH_DURATION + 10;
        uint256 rewardPoolId = _createRewardPoolWithExpiry(contentId, REWARD_POOL_AMOUNT, 3, expiresAt);
        ...
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-before-qualification"));

        vm.expectRevert("Round not qualified");
    ...
    function testPreQualificationRejectedRoundsCanBeAbandonedInBatches() public {
        ...
        vm.warp(expiresAt + 1);
        uint256 treasuryBalanceBefore = usdc.balanceOf(treasury);
        uint256 firstRefund = rewardPoolEscrow.refundPreQualificationRejectedRewardPool(
        ...
        uint256 secondRefund = rewardPoolEscrow.refundPreQualificationRejectedRewardPool(
            rewardPoolId, _singleRoundIds(secondRoundId), false
        );
    ...
    function testPreQualificationParentRejectedClusterSnapshotRoundCanBeSkippedAndRefunded() public {
        ...
        oracle.rejectFinalizedCorrelationEpoch(uint64(roundId), keccak256("reject-parent-before-qualification"));
        IClusterPayoutOracle.RoundPayoutSnapshot memory rejectedSnapshot =
            oracle.getRoundPayoutSnapshot(1, rewardPoolId, contentId, roundId);
        assertEq(uint8(rejectedSnapshot.status), uint8(IClusterPayoutOracle.SnapshotStatus.Rejected));
        assertFalse(oracle.rejectedRoundPayoutSnapshotDigests(snapshotKey, snapshotDigest));
        assertFalse(oracle.rejectedRoundPayoutSnapshotRoots(snapshotKey, root));
        assertTrue(oracle.isRoundPayoutSnapshotRejectedByCorrelationEpoch(1, rewardPoolId, contentId, roundId));
    ...
    function testPreQualificationParentRejectedClusterSnapshotRoundCanQualifyReplacementBeforeRefund() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        oracle.setOracleTimingConfig(1 hours, 2 hours);
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        ...
        oracle.rejectFinalizedCorrelationEpoch(uint64(roundId), keccak256("reject-parent-before-replacement"));
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotRound(rewardPoolId, roundId);
        ...
        oracle.proposeCorrelationEpoch(
            ...
            uint64(roundId),
            uint64(roundId),
            keccak256("replacement-parent-root"),
            keccak256("params"),
            keccak256("epoch-artifact"),
            "ipfs://replacement-epoch",
            _questionEpochSources(rewardPoolId, contentId, roundId)
        );
        vm.warp(block.timestamp + 1 hours + 1);
        oracle.finalizeCorrelationEpoch(uint64(roundId));

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("parent-rejection-prequalification-replacement");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _finalizeClusterRoundPayoutSnapshotWithRoot(
            oracle,
            rewardPoolId,
        ...
        assertGt(replacementSnapshot.allocation, 0);
    ...
    function testPreQualificationRejectedMismatchedReplacementDoesNotBlockRefund() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 expiresAt = block.timestamp + EPOCH_DURATION + 10;
        uint256 rewardPoolId = _createRewardPoolWithExpiry(contentId, REWARD_POOL_AMOUNT, 3, expiresAt);
        ...
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-before-mismatched-replacement"));
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotRound(rewardPoolId, roundId);

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("prequalification-mismatched-replacement");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _finalizeClusterRoundPayoutSnapshotWithRoot(
            oracle,
            rewardPoolId,
    ...
    function testPreQualificationRejectedClusterSnapshotRoundCanQualifyReplacementBeforeRefund() public {
        ...
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-before-replacement"));
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotRound(rewardPoolId, roundId);

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("prequalification-replacement");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _finalizeClusterRoundPayoutSnapshotWithRoot(
            oracle,
            rewardPoolId,
    ...
    // Legacy recovery bodies below model pre-finality deployments where a qualified snapshot
    // could still be rejected. Fresh deployments wait out the veto window before qualification.
    function legacyQualifiedClusterSnapshotRejectsReplacementRootClaims() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyQualifiedClusterSnapshotRejectsSameRootMetadataReplacementClaims() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyRecoveredClusterSnapshotCanReopenWithCorrectedSameRootMetadata() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyReopenRecoveredSnapshotRound_AllowsHonestRequalification() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyReopenRecoveredSnapshotRound_WaitsForReplacementVetoWindow() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        uint256 roundId = _settleRoundWith(_threeVoters(), contentId, _directions(true, true, false));
        ...
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-before-veto-window-test"));
        rewardPoolEscrow.recoverRejectedSnapshotRound(rewardPoolId, roundId);

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("replacement-inside-veto-window");
        _finalizeClusterRoundPayoutSnapshotWithRootNoVetoWait(
            oracle,
            rewardPoolId,
            contentId,
            ...
            30_000,
            replacementWeight.effectiveWeight,
            oracle.payoutWeightLeaf(replacementWeight)
        );
    ...
    function testReopenRecoveredSnapshotRoundRejectsNonQualifiableReplacement() public {
        _ensureTestClusterPayoutOracle(votingEngine);
        TestClusterPayoutOracle oracle = TestClusterPayoutOracle(protocolConfig.clusterPayoutOracle());
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        ...
        oracle.setFinalizedRoundPayoutSnapshot(1, rewardPoolId, contentId, roundId, 3, 20_000, 30_001);
    ...
    function testReopenRecoveredSnapshotRoundRejectsCurrentEligibilityDrift() public {
        _ensureTestClusterPayoutOracle(votingEngine);
        TestClusterPayoutOracle oracle = TestClusterPayoutOracle(protocolConfig.clusterPayoutOracle());
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyRecoveredSnapshotRoundRejectsAfterCompleteRefund() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 expiresAt = block.timestamp + EPOCH_DURATION + 10;
        uint256 rewardPoolId = _createRewardPoolWithExpiry(contentId, REWARD_POOL_AMOUNT, 3, expiresAt);
    ...
    function legacyReopenRecoveredSnapshotRound_RevertsWithoutNewOracleSnapshot() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
    ...
    function legacyRefundExpiredRecoveredRewardPool_AllowsExitAfterGraceWithoutReplacement() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        ...
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-for-recovered-refund"));
        ...
        uint256[] memory roundIds = _singleRoundIds(roundId);
        vm.expectRevert("Grace");
        rewardPoolEscrow.refundExpiredRecoveredRewardPool(rewardPoolId, roundIds);

        vm.warp(uint256(rewardPoolEscrow.rewardPoolRefundEligibleAt(rewardPoolId)) + 1);
        uint256 treasuryBefore = usdc.balanceOf(treasury);
        uint256 refundAmount = rewardPoolEscrow.refundExpiredRecoveredRewardPool(rewardPoolId, roundIds);

        assertEq(refundAmount, REWARD_POOL_AMOUNT);
        assertEq(usdc.balanceOf(treasury), treasuryBefore + REWARD_POOL_AMOUNT);
        assertFalse(rewardPoolEscrow.rejectedRecoveredRound(rewardPoolId, roundId));
        assertFalse(rewardPoolEscrow.reopenedRecoveredRound(rewardPoolId, roundId));
        RoundSnapshot memory cleared = rewardPoolEscrow.getRoundSnapshot(rewardPoolId, roundId);
        assertEq(cleared.allocation, 0);
        vm.expectRevert("Bounty refunded");
        rewardPoolEscrow.reopenRecoveredSnapshotRound(rewardPoolId, roundId);
    }

    function legacyRecoveredRoundPendingBlocksPreQualificationSkip() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        ...
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-before-prequalification-skip"));
        rewardPoolEscrow.recoverRejectedSnapshotRound(rewardPoolId, roundId);

        vm.expectRevert("Recovered round pending");
        rewardPoolEscrow.skipPreQualificationSnapshotlessClusterRound(rewardPoolId, roundId + 1);
    }

    function legacyRefundInactiveRecoveredRewardPool_AllowsExitWithoutReplacement() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        ...
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-for-inactive-recovered-refund"));
        rewardPoolEscrow.recoverRejectedSnapshotRound(rewardPoolId, roundId);
        ...
        uint256 treasuryBefore = usdc.balanceOf(treasury);
        uint256 refundAmount =
            rewardPoolEscrow.refundInactiveRecoveredRewardPool(rewardPoolId, _singleRoundIds(roundId));
    ...
    function testRepointRewardPoolOracleRejectsPriorLiveSnapshotForCurrentRound() public {
        ...
        vm.prank(owner);
        vm.expectRevert("Oracle snapshot exists");
        rewardPoolEscrow.repointRewardPoolClusterPayoutOracle(rewardPoolId, address(originalOracle));
    }

    function testRepointRewardPoolOracleAllowsPriorRejectedSnapshotForCurrentRound() public {
        ClusterPayoutOracle originalOracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        uint256 roundId = _settleRoundWith(_threeVoters(), contentId, _directions(true, true, false));
        ...
        bytes32 snapshotKey = originalOracle.roundPayoutSnapshotKey(1, rewardPoolId, contentId, roundId);
        originalOracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-before-repoint"));
    ...
    function testReplacementRewardPoolOracleCannotPreSquatBeforeRepointAndPostPinCanClaimAfterVetoWindow() public {
        _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        uint256 roundId = _settleRoundWith(_threeVoters(), contentId, _directions(true, true, false));
        ...
        IClusterPayoutOracle.PayoutWeight memory payoutWeight =
            _clusterPayoutWeight(rewardPoolId, contentId, roundId, 0);
        bytes32 weightRoot = replacementOracle.payoutWeightLeaf(payoutWeight);
        ...
        uint256 pinnedBlockTime = block.timestamp;
        replacementOracle.proposeCorrelationEpoch(
            uint64(roundId),
            uint64(roundId),
            uint64(roundId),
        ...
        replacementOracle.proposeRoundPayoutSnapshot(
            IClusterPayoutOracle.RoundPayoutSnapshotInput({
                domain: replacementOracle.PAYOUT_DOMAIN_QUESTION_REWARD(),
                rewardPoolId: rewardPoolId,
                contentId: contentId,
                roundId: roundId,
                correlationEpochId: uint64(roundId),
        ...
        assertEq(
            replacementOracle.roundPayoutSnapshotProposedAt(
                replacementOracle.PAYOUT_DOMAIN_QUESTION_REWARD(), rewardPoolId, contentId, roundId
            ),
            pinnedBlockTime
        );

        bytes32 snapshotKey = replacementOracle.roundPayoutSnapshotKey(
            replacementOracle.PAYOUT_DOMAIN_QUESTION_REWARD(), rewardPoolId, contentId, roundId
        );
        vm.warp(pinnedBlockTime + uint256(replacementOracle.challengeWindow()) + 1);
        replacementOracle.finalizeCorrelationEpoch(uint64(roundId));
        ...
        _warpPastQuestionRewardSnapshotVeto(replacementOracle, rewardPoolId, contentId, roundId);
    ...
    function testPermissionlessSkipSnapshotlessClusterRoundAndRefundExpiredPool() public {
        ...
        vm.prank(address(oracle));
        assertEq(rewardPoolEscrow.roundPayoutSnapshotSourceReadyAt(1, rewardPoolId, contentId, roundId), 0);
    ...
    function testSnapshotlessClusterSkipRejectsExistingOracleSnapshot() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        uint256 roundId = _settleRoundWith(_threeVoters(), contentId, _directions(true, true, false));
        ...
        oracle.proposeRoundPayoutSnapshot(
            IClusterPayoutOracle.RoundPayoutSnapshotInput({
                ...
                effectiveParticipantUnits: 30_000,
                totalClaimWeight: payoutWeight.effectiveWeight,
                weightRoot: oracle.payoutWeightLeaf(payoutWeight),
                reasonRoot: keccak256("reason-root"),
                artifactHash: keccak256("epoch-artifact"),
                artifactURI: "ipfs://round"
            })
    ...
    function testSnapshotlessSkippedClusterRoundCanRepointAndClaimReplacementSnapshot() public {
        _enableClusterPayoutOracle();
        uint256 contentId = _submitQuestion("");
        uint256 rewardPoolId = _createRewardPool(contentId, REWARD_POOL_AMOUNT, 3);
        uint256 roundId = _settleRoundWith(_threeVoters(), contentId, _directions(true, true, false));
        ...
        vm.prank(owner);
        vm.expectRevert("Bounty complete");
        rewardPoolEscrow.skipPreQualificationSnapshotlessClusterRound(rewardPoolId, roundId);
    }
    ...
    function testLaterPostWindowRoundCannotQualifyAfterSkippingIneligibleRound() public {
        ...
        vm.warp(block.timestamp + 25 hours);
        uint256 lateRoundId = _settleRoundWith(_threeVoters(), contentId, directions);

        (uint256 skipped, uint256 nextRoundToEvaluate) = rewardPoolEscrow.advanceQualificationCursor(rewardPoolId, 1);
        assertEq(skipped, 1);
        assertEq(nextRoundToEvaluate, lateRoundId);
        assertEq(rewardPoolEscrow.claimableQuestionReward(rewardPoolId, lateRoundId, voter1), 0);
    }
    ...
    function testAdvanceQualificationCursorSkipsIneligibleRoundsInChunks() public {
        ...
        assertEq(skipped, 1);
        assertEq(nextRoundToEvaluate, ineligibleRoundId + 1);
    }
    ...
    function testInactiveUnexpiredPoolCanRefundUnallocatedFunds() public {
        ...
        assertEq(refundAmount, REWARD_POOL_AMOUNT);
        assertEq(usdc.balanceOf(treasury), treasuryBalanceBefore + REWARD_POOL_AMOUNT);
    }
    ...
    function _defaultBundleWindowSeconds(uint256[] memory contentIds) internal view returns (uint256) {
        RoundLib.RoundConfig memory roundConfig = registry.getContentRoundConfig(contentIds[0]);
        return uint256(roundConfig.maxDuration);
    }
    ...
    function _challengeRoundPayoutSnapshot(ClusterPayoutOracle oracle, bytes32 snapshotKey, bytes32 reasonHash)
        internal
    {
        address challenger = address(0xCA11);
        uint256 challengeBond = oracle.challengeBond();
        usdc.mint(challenger, challengeBond);
        vm.startPrank(challenger);
        usdc.approve(address(oracle), challengeBond);
    ...
    function _finalizeClusterPayoutSnapshotWithRootNoVetoWait(
        ClusterPayoutOracle oracle,
        uint256 rewardPoolId,
        uint256 contentId,
        uint256 roundId,
    ...
    function _finalizeClusterRoundPayoutSnapshotWithRootNoVetoWait(
        ClusterPayoutOracle oracle,
        uint256 rewardPoolId,
        uint256 contentId,
        uint256 roundId,
    ...
    function _finalizeBundleClusterPayoutSnapshotWithRootNoVetoWait(
        ClusterPayoutOracle oracle,
        uint256 bundleId,
        uint256 roundSetIndex,
        uint32 rawEligibleVoters,
        ...
        _finalizeBundleCorrelationEpoch(oracle, bundleId, roundSetIndex);

        _finalizeBundleRoundPayoutSnapshotWithRootNoVetoWait(
            oracle,
            bundleId,
            roundSetIndex,
            snapshotRoundId,
    ...
    function _finalizeBundleRoundPayoutSnapshotWithRootNoVetoWait(
        ClusterPayoutOracle oracle,
        uint256 bundleId,
        uint256 roundSetIndex,
        uint64 correlationEpochId,
        ...
        bytes32 snapshotKey = _proposeBundleRoundPayoutSnapshotWithRoot(
            oracle,
            bundleId,
            roundSetIndex,
            correlationEpochId,
    ...
    function _finalizeBundleCorrelationEpoch(ClusterPayoutOracle oracle, uint256 bundleId, uint256 roundSetIndex)
        internal
    {
        uint64 snapshotRoundId = uint64(roundSetIndex + 1);
        oracle.proposeCorrelationEpoch(
    ...
    function _proposeBundleRoundPayoutSnapshotWithRoot(
        ClusterPayoutOracle oracle,
        uint256 bundleId,
        uint256 roundSetIndex,
        uint64 correlationEpochId,
        ...
        snapshotKey = oracle.roundPayoutSnapshotKey(
            oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), bundleId, bundleId, snapshotRoundId
        );
    }

    function _warpPastBundleRewardSnapshotVeto(ClusterPayoutOracle oracle, uint256 bundleId, uint256 roundSetIndex)
        internal
    {
        uint256 snapshotRoundId = roundSetIndex + 1;
        IClusterPayoutOracle.RoundPayoutSnapshot memory snapshot = oracle.getRoundPayoutSnapshot(
    ...
    function _settleBundleRoundSetWith(uint256[] memory contentIds, address[] memory voters, bool[] memory directions)
        internal
        returns (uint256 firstRoundId, uint256 secondRoundId)
    {
        return _settleBundleRoundSet(contentIds, voters, directions, true);
    }

    function _settleBundleRoundSetWithoutBundleSync(
        uint256[] memory contentIds,
        address[] memory voters,
        bool[] memory directions
    ) internal returns (uint256 firstRoundId, uint256 secondRoundId) {
        return _settleBundleRoundSet(contentIds, voters, directions, false);
    }

    function _settleBundleRoundSet(
        uint256[] memory contentIds,
        address[] memory voters,
        bool[] memory directions,
        bool syncBundle
    ) internal returns (uint256 firstRoundId, uint256 secondRoundId) {
        require(contentIds.length == 2, "test helper expects pair");
        CommittedRound memory first =
            _commitBundleQuestionRound(contentIds[0], voters, directions, keccak256("bundle-set-a"));
        CommittedRound memory second =
            _commitBundleQuestionRound(contentIds[1], voters, directions, keccak256("bundle-set-b"));

        uint256 firstRevealable = _lastCommitRevealableAfter(votingEngine, first.contentId, first.roundId);
        uint256 secondRevealable = _lastCommitRevealableAfter(votingEngine, second.contentId, second.roundId);
        _warpPastTlockRevealTime(firstRevealable > secondRevealable ? firstRevealable : secondRevealable);

        _revealCommittedVotes(first, directions, directions.length);
        _revealCommittedVotes(second, directions, directions.length);
        _settleCommittedRound(first, syncBundle);
        _settleCommittedRound(second, syncBundle);
        return (first.roundId, second.roundId);
    }

    function _revealAndSettleCommittedBundleRoundSet(
        CommittedRound memory first,
        bool[] memory firstDirections,
        CommittedRound memory second,
        bool[] memory secondDirections,
        bool syncBundle
    ) internal {
        uint256 firstRevealable = _lastCommitRevealableAfter(votingEngine, first.contentId, first.roundId);
        uint256 secondRevealable = _lastCommitRevealableAfter(votingEngine, second.contentId, second.roundId);
        ...
        _revealCommittedVotes(first, firstDirections, firstDirections.length);
        _revealCommittedVotes(second, secondDirections, secondDirections.length);
        _settleCommittedRound(first, syncBundle);
        _settleCommittedRound(second, syncBundle);
    }

    function _commitBundleQuestionRound(
        uint256 contentId,
        address[] memory voters,
        bool[] memory directions,
        bytes32 saltDomain
    ) internal returns (CommittedRound memory committed) {
        return _commitBundleQuestionRoundWithFrontend(contentId, voters, directions, saltDomain, address(0));
    }

    function _commitBundleQuestionRoundWithFrontend(
        uint256 contentId,
        address[] memory voters,
        bool[] memory directions,
        bytes32 saltDomain,
        ...
        assertEq(voters.length, directions.length);
        committed.contentId = contentId;
        committed.salts = new bytes32[](voters.length);
        committed.commitKeys = new bytes32[](voters.length);
        for (uint256 i = 0; i < voters.length; i++) {
            committed.salts[i] = keccak256(abi.encodePacked(saltDomain, voters[i], contentId, i, block.timestamp));
            committed.commitKeys[i] = _commitTestVote(
                DirectTestCommitRequest({
                    engine: votingEngine,
                    lrepToken: lrepToken,
                    voter: voters[i],
                    ...
                    stake: STAKE,
                    frontend: frontend,
                    salt: committed.salts[i]
                })
            );
        }
        committed.roundId = RoundEngineReadHelpers.activeRoundId(votingEngine, contentId);
    }

    function _revealAndSettleCommittedRound(CommittedRound memory committed, bool[] memory directions, bool syncBundle)
        internal
    {
        _revealCommittedVotes(committed, directions, directions.length);
        _settleCommittedRound(committed, syncBundle);
    }

    function _settleCommittedRound(CommittedRound memory committed, bool syncBundle) internal {
        _settleAfterRbtsSeed(votingEngine, committed.contentId, committed.roundId);
        if (syncBundle) {
            rewardPoolEscrow.syncBundleQuestionTerminal(committed.contentId, committed.roundId);
        }
    }

    function _revealCommittedVotes(CommittedRound memory committed, bool[] memory directions, uint256 revealCount)
        internal
    {
    ...
    function _roundSnapshotStorageSlot(uint256 rewardPoolId, uint256 roundId) internal pure returns (bytes32) {
        bytes32 rewardPoolSnapshotSlot = keccak256(abi.encode(rewardPoolId, uint256(6)));
    ...
    function testBundleRefundWindowAllowsThresholdReachedRoundsToSettleAfterClose() public {
        ...
        bool[] memory directions = _directions(true, true, false);
        CommittedRound memory first =
            _commitBundleQuestionRound(contentIds[0], voters, directions, keccak256("settle-after-close-a"));
        CommittedRound memory second =
            _commitBundleQuestionRound(contentIds[1], voters, directions, keccak256("settle-after-close-b"));
        uint256 firstRevealable = _lastCommitRevealableAfter(votingEngine, first.contentId, first.roundId);
        uint256 secondRevealable = _lastCommitRevealableAfter(votingEngine, second.contentId, second.roundId);
        _warpPastTlockRevealTime(firstRevealable > secondRevealable ? firstRevealable : secondRevealable);
        _revealCommittedVotes(first, directions, voters.length);
        _revealCommittedVotes(second, directions, voters.length);
        uint256 firstRoundId = first.roundId;
        uint256 secondRoundId = second.roundId;
        assertLe(_commitCommittedAt(votingEngine, contentIds[0], firstRoundId, first.commitKeys[0]), bountyClosesAt);
        assertLe(_commitCommittedAt(votingEngine, contentIds[1], secondRoundId, second.commitKeys[0]), bountyClosesAt);
    ...
    function testBundleRefundWindowCoversMaxRevealGraceForOpenThresholdRounds() public {
        ...
        bool[] memory directions = _directions(true, true, false, true);
        CommittedRound memory first =
            _commitBundleQuestionRound(contentIds[0], voters, directions, keccak256("grace-open-a"));
        CommittedRound memory second =
            _commitBundleQuestionRound(contentIds[1], voters, directions, keccak256("grace-open-b"));
        uint256 firstRevealable = _lastCommitRevealableAfter(votingEngine, first.contentId, first.roundId);
        uint256 secondRevealable = _lastCommitRevealableAfter(votingEngine, second.contentId, second.roundId);
        _warpPastTlockRevealTime(firstRevealable > secondRevealable ? firstRevealable : secondRevealable);
        _revealCommittedVotes(first, directions, voters.length - 1);
        _revealCommittedVotes(second, directions, voters.length - 1);
        uint256 firstRoundId = first.roundId;
        uint256 secondRoundId = second.roundId;
        assertLe(_commitCommittedAt(votingEngine, contentIds[0], firstRoundId, first.commitKeys[0]), bountyClosesAt);
        assertLe(_commitCommittedAt(votingEngine, contentIds[1], secondRoundId, second.commitKeys[0]), bountyClosesAt);
    ...
    function testBundleRefund_NoRecordedRoundSetStillWaitsForGraceAtBountyClose() public {
        ...
        uint256 firstRoundId = RoundEngineReadHelpers.activeRoundId(votingEngine, contentIds[0]);
        uint256 secondRoundId = RoundEngineReadHelpers.activeRoundId(votingEngine, contentIds[1]);
        votingEngine.cancelExpiredRound(contentIds[0], firstRoundId);
        votingEngine.cancelExpiredRound(contentIds[1], secondRoundId);
    ...
    function testBundleRefund_CompleteClusterRoundSetWaitsForFinalizedSnapshot() public {
        ...
        address[] memory voters = _threeVoters();
        bool[] memory directions = _directions(true, true, false);
        (uint256 firstRoundId,) = _settleBundleRoundSetWithoutBundleSync(contentIds, voters, directions);
        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
    ...
    function testBundleRefund_SnapshotlessClusterRoundSetCanBeSkipped() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        uint256 bountyClosesAt = block.timestamp + EPOCH_DURATION;
        ...
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotBundleRoundSet(bundleId, 0);
        uint256 treasuryBalanceBefore = usdc.balanceOf(treasury);
        uint256 refundAmount = rewardPoolEscrow.refundQuestionBundleReward(bundleId);
    ...
    function testBundleRefund_SnapshotlessSkipRejectsNonQualifyingCompleterIntersection() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundleWithClose(
            contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3, block.timestamp + 3 days
        );
        ...
        uint256 firstRoundId = _settleRoundWithoutBundleSync(firstQuestionVoters, contentIds[0], directions);
        uint256 secondRoundId = _settleRoundWithoutBundleSync(secondQuestionVoters, contentIds[1], directions);
        _recordBundleRoundSetTerminals(contentIds, firstRoundId, secondRoundId);

        vm.expectRevert("Round set not qualifiable");
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotBundleRoundSet(bundleId, 0);
        ...
        vm.warp(block.timestamp + 25 hours);
        (uint256 replacementFirstRoundId, uint256 replacementSecondRoundId) =
            _settleBundleRoundSetWithoutBundleSync(contentIds, _threeVoters(), directions);
        _recordBundleRoundSetTerminals(contentIds, replacementFirstRoundId, replacementSecondRoundId);

        IClusterPayoutOracle.PayoutWeight memory replacementWeight =
            _bundlePayoutWeight(bundleId, 0, contentIds[0], replacementFirstRoundId, 0);
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _finalizeBundleClusterPayoutSnapshotWithRoot(
            oracle, bundleId, 0, 3, 30_000, replacementWeight.effectiveWeight, replacementRoot
        );
        rewardPoolEscrow.syncQuestionBundleTerminals(bundleId, 10);
    ...
    function testBundleRefund_SnapshotlessClusterRoundSetRejectsExistingOracleSnapshot() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        vm.expectRevert("Snapshot not rejected");
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotBundleRoundSet(bundleId, 0);
    }

    function testBundleRefund_PreQualificationRejectedSnapshotRoundSetCanBeSkipped() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        uint256 bountyClosesAt = block.timestamp + EPOCH_DURATION;
        ...
        bytes32 snapshotKey =
            oracle.roundPayoutSnapshotKey(oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), bundleId, bundleId, 1);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-bundle-before-qualification"));

        vm.expectRevert("Round set not qualified");
        rewardPoolEscrow.recoverOrReopenSnapshotBundleRoundSet(bundleId, 0);
        ...
        assertEq(refundAmount, REWARD_POOL_AMOUNT);
        assertEq(usdc.balanceOf(treasury), treasuryBalanceBefore + refundAmount);
        assertFalse(rewardPoolEscrow.isRoundPayoutSnapshotConsumed(4, bundleId, bundleId, 1));
    }

    function testBundleRefund_PreQualificationRejectedSkipRejectsRawCompleterMismatch() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        bytes32 snapshotKey =
            oracle.roundPayoutSnapshotKey(oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), bundleId, bundleId, 1);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-bundle-mismatched-raw"));

        vm.expectRevert("Round set not qualifiable");
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotBundleRoundSet(bundleId, 0);
    }

    function testBundleRefund_PreQualificationRejectedSkipRejectsBelowFloorSnapshot() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        bytes32 root = oracle.payoutWeightLeaf(payoutWeight);
        _finalizeBundleClusterPayoutSnapshotWithRootNoVetoWait(
            oracle, bundleId, 0, 3, 20_000, payoutWeight.effectiveWeight, root
        );

        bytes32 snapshotKey =
            oracle.roundPayoutSnapshotKey(oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), bundleId, bundleId, 1);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-bundle-below-floor"));

        vm.expectRevert("Round set not qualifiable");
        rewardPoolEscrow.skipPreQualificationRejectedSnapshotBundleRoundSet(bundleId, 0);
    }

    function testBundleRefund_PreQualificationParentRejectedSnapshotRoundSetCanBeSkipped() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        oracle.setOracleTimingConfig(1 hours, 2 hours);
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        uint8 bundlePayoutDomain = oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD();
        bytes32 snapshotKey = oracle.roundPayoutSnapshotKey(bundlePayoutDomain, bundleId, bundleId, 1);
        bytes32 snapshotDigest = oracle.roundPayoutSnapshotProposalDigest(snapshotKey);
        oracle.rejectFinalizedCorrelationEpoch(1, keccak256("reject-bundle-parent-before-qualification"));
        IClusterPayoutOracle.RoundPayoutSnapshot memory rejectedSnapshot =
            oracle.getRoundPayoutSnapshot(bundlePayoutDomain, bundleId, bundleId, 1);
        assertEq(uint8(rejectedSnapshot.status), uint8(IClusterPayoutOracle.SnapshotStatus.Rejected));
        assertFalse(oracle.rejectedRoundPayoutSnapshotDigests(snapshotKey, snapshotDigest));
        assertFalse(oracle.rejectedRoundPayoutSnapshotRoots(snapshotKey, root));
        assertTrue(oracle.isRoundPayoutSnapshotRejectedByCorrelationEpoch(bundlePayoutDomain, bundleId, bundleId, 1));
        ...
        assertEq(refundAmount, REWARD_POOL_AMOUNT);
        assertEq(usdc.balanceOf(treasury), treasuryBalanceBefore + refundAmount);
        assertFalse(rewardPoolEscrow.isRoundPayoutSnapshotConsumed(bundlePayoutDomain, bundleId, bundleId, 1));
    }

    function testBundleRefund_PreQualificationRejectedSnapshotRoundSetCanQualifyReplacementBeforeRefund() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        bytes32 snapshotKey =
            oracle.roundPayoutSnapshotKey(oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), bundleId, bundleId, 1);
        oracle.rejectFinalizedRoundPayoutSnapshot(snapshotKey, keccak256("reject-bundle-before-replacement"));

        rewardPoolEscrow.skipPreQualificationRejectedSnapshotBundleRoundSet(bundleId, 0);
        uint8 bundlePayoutDomain = oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD();
        vm.prank(address(oracle));
        assertGt(rewardPoolEscrow.roundPayoutSnapshotSourceReadyAt(bundlePayoutDomain, bundleId, bundleId, 1), 0);

        IClusterPayoutOracle.PayoutWeight memory replacementWeight = payoutWeight;
        replacementWeight.reasonHash = keccak256("prequalification-bundle-replacement");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _finalizeBundleRoundPayoutSnapshotWithRoot(
            oracle, bundleId, 0, uint64(1), 3, 30_000, replacementWeight.effectiveWeight, replacementRoot
        );
    ...
    function testBundleRefund_SnapshotlessSkippedRoundSetCanQualifyReplacementBeforeRefund() public {
        ClusterPayoutOracle oracle = _enableClusterPayoutOracle();
        uint256[] memory contentIds = _submitBundleQuestions();
        uint256 bundleId = _createSubmissionBundle(contentIds, funder, REWARD_ASSET_USDC, REWARD_POOL_AMOUNT, 3);
        ...
        IClusterPayoutOracle.PayoutWeight memory replacementWeight =
            _bundlePayoutWeight(bundleId, 0, contentIds[0], firstRoundId, 0);
        replacementWeight.reasonHash = keccak256("snapshotless-bundle-replacement");
        bytes32 replacementRoot = oracle.payoutWeightLeaf(replacementWeight);
        _finalizeBundleClusterPayoutSnapshotWithRoot(
            oracle, bundleId, 0, 3, 30_000, replacementWeight.effectiveWeight, replacementRoot
        );
        ...
        uint256 paid = rewardPoolEscrow.claimQuestionBundleReward(bundleId, 0, replacementWeight, new bytes32[](0));
        assertGt(paid, 0);
        assertTrue(rewardPoolEscrow.isRoundPayoutSnapshotConsumed(bundlePayoutDomain, bundleId, bundleId, 1));
    }
    ...
    function testBundleRefund_BelowFloorClusterRoundSetDoesNotPinFunds() public {
        ...
        uint256 vetoDeadline = uint256(payoutSnapshot.finalizedAt) + uint256(oracle.FINALIZATION_VETO_WINDOW());
        vm.warp(vetoDeadline - 1);
        vm.expectRevert(QuestionRewardPoolEscrowBundleActionsLib.BundleClusterPayoutSnapshotPending.selector);
        rewardPoolEscrow.refundQuestionBundleReward(bundleId);
    ...
    function testBundleRefund_EmptyClusterRoundSetDoesNotPinFunds() public {
        ...
        vm.warp(bountyClosesAt + BUNDLE_REFUND_GRACE + 1);
        _finalizeBundleClusterPayoutSnapshotWithRootNoVetoWait(oracle, bundleId, 0, 3, 0, 0, bytes32(0));
        IClusterPayoutOracle.RoundPayoutSnapshot memory payoutSnapshot =
            oracle.getRoundPayoutSnapshot(oracle.PAYOUT_DOMAIN_QUESTION_BUNDLE_REWARD(), bundleId, bundleId, 1);
    ...
    function testBundleRefund_ClaimGraceBlocksRaceAtBountyClose() public {
        ...
        // Jump past the creation-time bundle close.
        vm.warp(bountyClosesAt + 1);
        vm.expectRevert("Grace");
        rewardPoolEscrow.refundQuestionBundleReward(bundleId);

        // Jump past the refund grace; refund is now allowed (forfeits to treasury since
    ...
    function testBundleRefund_LateQualificationStartsClaimGraceAfterRefundGrace() public {
        ...
        bool[] memory firstDirections = _directions(true, true, false, true);
        CommittedRound memory first =
            _commitBundleQuestionRound(contentIds[0], firstVoters, firstDirections, keccak256("cleanup-pending-a"));
        address[] memory voters = _threeVoters();
        bool[] memory directions = _directions(true, true, false);
        CommittedRound memory second =
            _commitBundleQuestionRound(contentIds[1], voters, directions, keccak256("cleanup-pending-b"));
        uint256 firstRevealable = _lastCommitRevealableAfter(votingEngine, first.contentId, first.roundId);
        uint256 secondRevealable = _lastCommitRevealableAfter(votingEngine, second.contentId, second.roundId);
        _warpPastTlockRevealTime(firstRevealable > secondRevealable ? firstRevealable : secondRevealable);

        _revealCommittedVotes(first, firstDirections, firstVoters.length - 1);
        _revealCommittedVotes(second, directions, voters.length);
