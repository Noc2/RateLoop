...
contract RoundVotingEngineRbtsSettlementModule is RoundVotingEngineStorage {
    ...
    bytes32 public constant RATELOOP_RBTS_SETTLEMENT_MODULE_MARKER = keccak256("rateloop.rbts-settlement-module.v1");
        ...
        view
    {
        ...
        IClusterPayoutOracle oracle = IClusterPayoutOracle(oracleAddress);
        bytes32 snapshotKey;
        try oracle.roundPayoutSnapshotKey(PAYOUT_DOMAIN_RBTS_SETTLEMENT, 0, contentId, roundId) returns (bytes32 key) {
            snapshotKey = key;
        } catch {
            return;
        }
