...
interface IQuestionRewardPoolEscrow {
    ...
    /// @notice Autogenerated public-state getter on QuestionRewardPoolEscrow. Used by
    ///         ContentRegistry.setQuestionRewardPoolEscrow as an ABI-shape probe (I-Identity-A).
    function defaultFrontendFeeBps() external view returns (uint16);
}

interface IQuestionBundleRewardPoolEscrow {
    function createSubmissionBundleFromRegistry(
        uint256 bundleId,
        uint256[] calldata contentIds,
        address funder,
    ...
    /// @notice Autogenerated public-state getter on QuestionRewardPoolEscrow-compatible escrows.
    function defaultFrontendFeeBps() external view returns (uint16);
}
...
contract ContentRegistry is Initializable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardTransient {
    ...
    /// @notice Optional escrow used only for question bundles. Falls back to `questionRewardPoolEscrow` when unset.
    address public questionBundleRewardPoolEscrow;
    address public confidentialityEscrow;
    /// @notice Bundle escrow snapshotted at submission so terminal callbacks keep using the correct bundle owner.
    mapping(uint256 => address) public questionBundleRewardPoolEscrowForBundle;
    ...
    event QuestionBundleRewardPoolEscrowUpdated(address rewardPoolEscrow);
    ...
    function setQuestionRewardPoolEscrow(address _questionRewardPoolEscrow) external onlyRole(CONFIG_ROLE) {
        ...
        // I-Identity-A: align defense-in-depth with `setCategoryRegistry` /
        // `setProtocolConfig` — probe a known autogenerated getter on the candidate to confirm
        // the ABI shape before pointing the registry at it. Pause-gating still protects against
        // a malicious-governance fat-finger; the probe rejects an outright incompatible address
        // (e.g. wrong contract pinned).
        try IQuestionRewardPoolEscrow(_questionRewardPoolEscrow).defaultFrontendFeeBps() returns (uint16) { }
        catch {
    ...
    function setQuestionBundleRewardPoolEscrow(address _questionBundleRewardPoolEscrow) external onlyRole(CONFIG_ROLE) {
        require(_questionBundleRewardPoolEscrow != address(0), "Invalid address");
        require(_questionBundleRewardPoolEscrow.code.length != 0, "No code");
        try IQuestionBundleRewardPoolEscrow(_questionBundleRewardPoolEscrow).defaultFrontendFeeBps() returns (
            uint16
        ) { }
        catch {
            revert("Invalid escrow");
        }
        if (questionBundleRewardPoolEscrow == _questionBundleRewardPoolEscrow) return;
        require(questionBundleRewardPoolEscrow == address(0) || paused(), "Pause required");
        questionBundleRewardPoolEscrow = _questionBundleRewardPoolEscrow;
        emit QuestionBundleRewardPoolEscrowUpdated(_questionBundleRewardPoolEscrow);
    }

    function setConfidentialityEscrow(address _confidentialityEscrow) external onlyRole(CONFIG_ROLE) {
        if (_confidentialityEscrow != address(0)) {
            require(_confidentialityEscrow.code.length != 0, "No code");
            try IConfidentialityEscrow(_confidentialityEscrow).confidentialityConfig(0) returns (
                IConfidentialityEscrow.ConfidentialityConfig memory
            ) { }
            catch {
                revert("Invalid confidentiality escrow");
            }
        }
        confidentialityEscrow = _confidentialityEscrow;
        emit ConfidentialityEscrowUpdated(_confidentialityEscrow);
    }
    ...
    function submitQuestionBundleWithRewardAndRoundConfig(
        ...
        address bundleRewardPoolEscrow = _activeQuestionBundleRewardPoolEscrow();
        require(bundleRewardPoolEscrow != address(0), "Bounty escrow not set");
        ...
        questionBundleRewardPoolEscrowForBundle[bundleId] = bundleRewardPoolEscrow;
        ...
        uint256 rewardPoolId = IQuestionBundleRewardPoolEscrow(bundleRewardPoolEscrow)
    ...
    function _activeQuestionBundleRewardPoolEscrow() internal view returns (address bundleRewardPoolEscrow) {
        bundleRewardPoolEscrow = questionBundleRewardPoolEscrow;
        if (bundleRewardPoolEscrow == address(0)) {
    ...
    function questionBundleRoundObserver(uint256 contentId) external view returns (address bundleRewardPoolEscrow) {
        ...
        if (bundleId != 0) {
            bundleRewardPoolEscrow = questionBundleRewardPoolEscrowForBundle[bundleId];
        }
        if (bundleRewardPoolEscrow == address(0)) {
            bundleRewardPoolEscrow = questionRewardPoolEscrow;
        }
    ...
    function submitQuestionFromX402Gateway(
        ...
        (uint256 resolvedCategoryId, bytes32 submissionKey, PendingSubmission memory pending) = _prepareQuestionMediaSubmission(
            metadata, imageUrls, videoUrl, details, salt, rewardTerms, validatedRoundConfig, spec, submitter, bytes32(0)
        );
    ...
    function _configureConfidentiality(
        ...
        require(confidentialityEscrow != address(0), "Confidentiality escrow not set");
        IConfidentialityEscrow(confidentialityEscrow).configure(contentId, confidentiality);
    }
