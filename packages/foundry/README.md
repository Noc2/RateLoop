...
Deployment requires `BASE_SEPOLIA_RPC_URL`, `TOKENLESS_ROTATION_AUTHORITY`, and `TOKENLESS_INITIAL_SIGNER`. The rotation authority must expose the Safe-compatible `getOwners()` and `getThreshold()` views, contain at least three distinct nonzero owners, and require at least two approvals. Before account selection or keystore unlock, the wrapper verifies RPC chain ID `84532`, deployed authority bytecode, and the complete Safe owner/threshold policy; an EOA or 1-of-N authority fails with an actionable configuration error. Losing that authority requires abandoning and freshly deploying the complete bundle; compromise permits emergency admission-signer rotation and can affect new commits until the bundle or authority is secured, but grants no fund-core authority. Before any transaction is broadcast, the deploy-profile build must be warning-free and a hard gate checks the exact five deployment runtimes against EIP-170 and their complete initcodes, including constructor arguments, against EIP-3860. The Solidity script repeats the authority and size checks before `vm.startBroadcast()`. After a successful broadcast it exports the isolated deployment JSON and regenerates only the tokenless TypeScript artifacts.
