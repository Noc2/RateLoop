...
Deployment requires `BASE_SEPOLIA_RPC_URL`, `TOKENLESS_ROTATION_AUTHORITY`, and `TOKENLESS_INITIAL_SIGNER`. The wrapper verifies RPC chain ID `84532` before broadcasting. Before a keystore is opened or any transaction is broadcast, the deploy-profile build must be warning-free and a hard gate checks the exact four deployment runtimes against EIP-170 and their complete initcodes, including constructor arguments, against EIP-3860. The Solidity script repeats those checks before `vm.startBroadcast()`. After a successful broadcast it exports the isolated deployment JSON and regenerates only the tokenless TypeScript artifacts.
