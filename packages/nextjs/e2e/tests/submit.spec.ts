...
async function expectSuccessfulSubmission(page: Page) {
  ...
  await expect(viewContentLink).toHaveClass(/rateloop-gradient-action/);
  await expect(viewContentLink).toHaveClass(/!w-full/);
  await expect(copyLinkButton).toHaveClass(/btn-outline/);
  await expect(copyLinkButton).toHaveClass(/w-full/);
  await expect(copyLinkButton).toHaveClass(/gap-2/);
