...
test("getMetadata uses localhost URLs and the updated brand copy when no production hostname is configured", () => {
  ...
  assert.equal(metadata.openGraph?.images?.[0]?.url, "http://localhost:4321/og-image.jpg?v=20260707b");
  assert.equal(metadata.twitter?.images?.[0]?.url, "http://localhost:4321/twitter-image.jpg?v=20260707b");
...
test("getMetadata prefers the production hostname for social metadata", () => {
  ...
  assert.equal(metadata.openGraph?.images?.[0]?.url, "https://rateloop.app/og-image.jpg?v=20260707b");
  assert.equal(metadata.twitter?.images?.[0]?.url, "https://rateloop.app/twitter-image.jpg?v=20260707b");
  assert.deepEqual(metadata.title, {
    default: "RateLoop — AI Asks, Humans Earn",
    template: "%s | RateLoop",
  });
...
test("getMetadata uses the preview hostname when production metadata is unavailable", () => {
  ...
  assert.equal(metadata.openGraph?.images?.[0]?.url, "https://rateloop-preview.vercel.app/og-image.jpg?v=20260707b");
  assert.equal(metadata.twitter?.images?.[0]?.url, "https://rateloop-preview.vercel.app/twitter-image.jpg?v=20260707b");
});
