...
function eligibleRow(overrides: Record<string, unknown> = {}) {
  return {
    ...
    risk_expires_at: new Date(NOW.getTime() + 43_200_000),
...
test("paid-review preflight fails closed for every legal, tax, sanctions, payout, and wallet gate", async () => {
  ...
  for (const changed of [
    ...
    { dac7_status: "complete", dac7_record_ciphertext: null },
    { risk_geoblock_status: "blocked" },
    { risk_plausibility_status: "review" },
    { wallet_screening_status: "review" },
    { risk_expires_at: NOW },
