...
export async function requireSignedInRaterSession(request: NextRequest, requireSameOrigin: boolean) {
...
export async function requireRaterSession(request: NextRequest, requireSameOrigin: boolean) {
  const session = await requireSignedInRaterSession(request, requireSameOrigin);
  return { ...session, payoutAddress: await requireRaterPayoutAddress(session.principalId) };
}
