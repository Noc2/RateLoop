...
function policy() {
  return {
    ...
    assurance: { requiredCapabilities: ["customer_invitation" as const], allowedProviders: [] },
    buyerPrivacy: {
...
test("run orchestration freezes rubric, blinded cases, policy hash, rounds, aggregate state, and rerun lineage", async () => {
  ...
  for (const [index, choice] of ["candidate", "candidate"].entries()) {
    await dbClient.execute({
      ...
      args: [`response_${index}`, run.runId, suite.caseId, `reviewer_${index}`, choice, `digest_${index}`, now, now],
    });
  }
  ...
  for (const status of ["recruiting", "collecting", "aggregating", "completed"] as const) {
