...
export async function listDirectPrivateReviewAssignments(input: {
  ...
  const paidReviewCompleted = paidReviewCompletionSql("rp.compensation_mode", "a.assignment_id");
  const result = await dbClient.execute({
    sql: `SELECT a.assignment_id,a.status,a.reservation_expires_at,a.assignment_expires_at,
                 ...
                 CASE WHEN ${paidReviewCompleted} THEN 'confirmed' ELSE NULL END AS paid_commit_state
