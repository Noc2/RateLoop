...
export const protocolCopy = {
  ...
  strongerSignalOverview:
    "Curyo offers a stronger signal: question-first submissions with a mandatory non-refundable bounty, a required context URL, optional preview media, and stake-weighted predicted ratings from open raters, recorded publicly and backed by economic risk.",
  predictionGamesOverview: `Curyo replaces passive likes with question-first prediction rounds and a mandatory non-refundable bounty. Submissions start as a question and require a context URL; optional preview media can be attached when it helps discovery. The attached bounty is funded in MREP or USDC on Celo and displayed to users in the funding asset, while raters predict the content's final 0-10 rating and back those predictions with MREP token stakes. Nearer revealed predictions win the content-specific rater pool: revealed misses can reclaim ${protocolDocFacts.revealedLoserRefundPercentLabel} of raw stake, and the remaining losing pool is split across accurate raters, frontend fees, consensus reserve, and treasury according to fixed on-chain percentages. There is no submitter upside path.`,
  contributorRewardsOverview: `After a ${protocolDocFacts.revealedLoserRefundPercentLabel} rebate for revealed misses, the remaining losing stake funds the content-specific rater pool plus frontend, consensus, and treasury shares. Bounties are attached at submission, funded in MREP or USDC on Celo, remain non-refundable, and pay eligible revealed raters with a 3% commit-attributed frontend-operator share inside each qualified bounty round. People, bots, and AI agents submit through the same path when they want public feedback.`,
  ...
  participationPoolPurpose:
    "Bootstraps early adoption -- calibration and rater bootstrap rewards become claimable after round settlement, and the rate halves based on cumulative MREP distributed from the pool.",
  participationPoolOverview:
    "The Bootstrap Pool solves the cold start problem. When the platform is new and stakes are small, round rewards alone may not be enough to attract raters. The Bootstrap Pool pays proportional MREP bonuses based on stake amount and prediction accuracy: accurate revealed raters claim bootstrap rewards after round settlement, and the reward rate is snapshotted at resolution time for fairness. Early participants receive the most thanks to a halving schedule as cumulative rewards grow and the reward rate decreases.",
  governanceOverview:
