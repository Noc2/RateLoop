...
test("processes local development image uploads without Vercel Blob", async () => {
  ...
  try {
    ...
    const feedStored = await readLocalImageAttachment(
      getImageAttachmentVariantPathname(attachment?.normalizedBlobPathname ?? "", "feed"),
    );
    const previewStored = await readLocalImageAttachment(
      getImageAttachmentVariantPathname(attachment?.normalizedBlobPathname ?? "", "preview"),
    );
    assert.ok(feedStored?.buffer.length);
    assert.ok(previewStored?.buffer.length);
  } finally {
...
test("creates approved blob-backed image attachments directly from generated bytes", async () => {
  ...
  try {
    ...
    assert.deepEqual(putPathnames, [
      normalizedPathname,
      `question-attachments/${attachmentId}/image-feed.webp`,
      `question-attachments/${attachmentId}/image-preview.webp`,
    ]);
...
test("sweeps expired unattached image uploads and deletes stored files", async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(tempDir, "question-attachments", "att_expiredorphan001", "image-feed.webp"),
      "expired-feed",
    );
    await writeFile(path.join(tempDir, "question-attachments", "att_retainedorphan01", "image.webp"), "retained");
    await writeFile(
      path.join(tempDir, "question-attachments", "att_retainedorphan01", "image-feed.webp"),
      "retained-feed",
    );
    ...
    assert.equal(await readLocalImageAttachment(getImageAttachmentVariantPathname(expiredPathname, "feed")), null);
    assert.ok(await readLocalImageAttachment(retainedPathname));
    assert.ok(await readLocalImageAttachment(getImageAttachmentVariantPathname(retainedPathname, "feed")));
  } finally {
    if (originalLocalImageDir === undefined) {
      delete process.env.RATELOOP_LOCAL_IMAGE_ATTACHMENT_DIR;
    } else {
