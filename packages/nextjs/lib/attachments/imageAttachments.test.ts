...
test("builds RateLoop upload image URLs with a webp extension", () => {
  assert.equal(
    getAttachmentImageUrl("https://www.rateloop.ai/ask", "att_abcdefghijklmnop"),
...
test("creates approved local image attachments directly from generated bytes", async () => {
  ...
  try {
    ...
    assert.equal(result.imageUrl, `https://www.rateloop.ai/api/attachments/images/${attachmentId}.webp`);
...
test("rejects too many image URLs before submission", async () => {
  assert.equal(
    await getImageAttachmentSubmissionValidationError({
      imageUrls: [
        "https://www.rateloop.ai/api/attachments/images/att_abcdefghijklmnop.webp",
        "https://www.rateloop.ai/api/attachments/images/att_bcdefghijklmnopq.webp",
        "https://www.rateloop.ai/api/attachments/images/att_cdefghijklmnopqr.webp",
        "https://www.rateloop.ai/api/attachments/images/att_defghijklmnopqrs.webp",
        "https://www.rateloop.ai/api/attachments/images/att_efghijklmnopqrst.webp",
      ],
      ownerWalletAddress: "0x00000000000000000000000000000000000000aa",
    }),
