...
function buildTreasuryGrantDescription(values: Record<string, string>) {
  ...
  return [
    `Treasury grant: ${amount} LREP to ${recipient}`,
    "",
    `Track: ${track}`,
    ...
    "",
    "This proposal transfers LREP from the governance timelock treasury to the recipient. LREP carries voting power and is intended for protocol-aligned ecosystem participation, not as a protocol-backed payment.",
  ].join("\n");
}
...
const actionTemplates: readonly GovernanceActionTemplate[] = [
  ...
  },
  {
    ...
    buildDescription: values => `Set proposal threshold to ${values.threshold || "0"} LREP`,
  },
  {
  ...
  },
  {
    ...
    description: "Create a proposal to send LREP from the governance timelock treasury to a recipient.",
    ...
    fields: [
      ...
      },
      {
        ...
        type: "textarea",
        required: true,
        helperText: "Explain why this recipient should hold LREP.",
      },
      {
        key: "impact",
  ...
  {
    id: "protocol-set-rater-registry",
    group: "Protocol Config",
    label: "Set rater registry",
    mode: "proposal",
    contractName: "ProtocolConfig",
    functionName: "setRaterRegistry",
    description: "Create a proposal to update the rater identity and credential registry used by protocol policy.",
    advanced: true,
    fields: [{ key: "raterRegistry", label: "RaterRegistry address", type: "address", required: true }],
    buildArgs: (_, parser) => [parser.address("raterRegistry", "RaterRegistry address")],
  ...
  const parser: FieldParser = {
    ...
    lrep: (key, label) => {
      ...
      try {
        ...
        throw new Error(`${label} must be a valid LREP amount.`);
      }
    },
    bytes32: (key, label) => {
      const value = formValues[key]?.trim() ?? "";
              ...
              {isTreasuryGrant && (
                <div className="space-y-2 pt-2">
                  ...
                  {grantExceedsTreasury && (
                    <p className="text-base text-warning">
                      The grant amount exceeds the current timelock LREP balance, so execution would fail unless the
                      treasury receives more LREP before execution.
                    </p>
                  )}
                </div>
              )}
