...
test("the app mounts one shared legacy-style notification surface", () => {
  assert.match(appProvidersSource, /<RateLoopNotificationProvider dismissLabel=\{notificationDismissLabel\}>/);
  assert.match(rootLayoutSource, /shared\.notifications\.dismiss/);
  ...
  assert.match(providerSource, /dismissLabel = "Dismiss notification"/);
  assert.match(providerSource, /aria-label=\{dismissLabel\}/);
