...
export function buildAgentConnectionMessage({ connectionUrl }: AgentConnectionMessageInput) {
  return `[@RateLoop Workspace](plugin://rateloop-workspace@rateloop) Use $rateloop-workspace-connection to connect this agent to my workspace and finish automatically in this task. Treat this as a new connection if a previous RateLoop workspace was deleted. Preserve this link privately. Only interrupt me for a host-presented install, trust, or OAuth action, or once if no workspace tools appear and I must uninstall existing RateLoop plugins. After I complete the action, resume through the host's Continue action when offered, check for RateLoop workspace tools on the next active turn, and continue automatically; never ask me to paste the link or approve the same action again. Report success only after RateLoop verifies the connection: ${connectionUrl}`;
}
