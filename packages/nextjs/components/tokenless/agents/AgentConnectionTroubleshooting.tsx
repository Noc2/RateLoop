  ...
  return (
    <details className="group mt-4 border-l border-base-content/20 py-1 pl-4 text-sm open:border-base-content/45">
      ...
      <p className="pb-3 pr-4 leading-6 text-base-content/60">
        New installs authorize RateLoop before the connection task starts. If Codex shows “Authentication complete” for
        an existing install, return to the same task and use Continue if offered; you should not need to type another
        message. If the tools are still missing on a later turn and Codex offers no action, uninstall both RateLoop
        plugins—<code>rateloop</code> and <code>rateloop-workspace</code>—then resume the same task with the original
        connection message.
      </p>
    </details>
  );
