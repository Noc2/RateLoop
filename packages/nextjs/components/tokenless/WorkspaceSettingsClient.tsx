...
function usdc(value: string, locale = "en") {
  return new Intl.NumberFormat(locale, { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(
    Number(BigInt(value)) / 1_000_000,
  );
}

function signedUsdc(value: string, locale = "en") {
  const atomic = BigInt(value);
  return `${atomic >= 0n ? "+" : "-"}$${usdc((atomic >= 0n ? atomic : -atomic).toString(), locale)}`;
}

function dateLabel(value: string | null, locale = "en") {
  if (!value) return "—";
...
export function WorkspaceSettingsClient({ initialWorkspaceId = "" }: { initialWorkspaceId?: string }) {
  ...
  useEffect(() => {
    void loadBilling(selectedId).catch(() => {
      ...
      setBilling(null);
      setBillingError("Unable to load billing status.");
    });
  ...
  useEffect(() => {
    ...
    void loadTopups(workspaceId).catch(() => {
      if (!workspaceRequests.isWorkspaceCurrent(workspaceId)) return;
      setTopupError("Unable to load prepaid funding.");
    });
  ...
  useEffect(() => {
    ...
    const workspaceId = selectedId;
    void loadIdentity(workspaceId).catch(() => {
      if (!workspaceRequests.isWorkspaceCurrent(workspaceId)) return;
      setIdentityError("Unable to load enterprise identity.");
    });
  ...
  async function openBillingDestination(kind: "checkout" | "portal") {
    ...
    try {
      ...
      setBillingError("Unable to open billing.");
  ...
  async function createTopup(event: FormEvent) {
    ...
    try {
      ...
      if (request.isCurrent()) {
        setTopupError("Unable to request a prepaid top-up.");
      }
    } finally {
  ...
  async function saveIdentityProvider(event: FormEvent) {
    ...
    try {
      ...
      setIdentityError("Unable to save the identity provider.");
    } finally {
      setIdentityBusy(false);
    }
  }
  ...
  async function identityProviderAction(
    ...
    try {
      ...
      setIdentityError("Unable to update the identity provider.");
    } finally {
      setIdentityBusy(false);
    }
  }
  ...
  async function scimAction(providerId?: string) {
    ...
    try {
      ...
      setIdentityError("Unable to update SCIM provisioning.");
    } finally {
      setIdentityBusy(false);
    }
  }
            ...
            <>
              {selected && canManageWorkspace ? (
                <>
                  ...
                  <section
                    className="mt-5 rounded-xl border border-base-content/10 p-5"
                    aria-labelledby="workspace-api-access"
                  >
              ...
              <section
                ...
                className={`${selected && canManageWorkspace ? "mt-5 " : ""}rounded-xl border border-base-content/10 bg-base-content/[0.025] p-5`}
              >
                ...
                {billingWarning ? (
                  <div className="mt-4 rounded-lg border border-warning/25 bg-warning/[0.07] p-3 text-sm leading-6 text-warning">
                    Payment needs attention. Existing accepted work can finish; update the payment method to keep
                    starting Early Access work.
                  </div>
                ) : null}
                  ...
                  <>
                    ...
                    {showPlanComparison ? (
                      <div
                        ...
                        className="mt-5 grid gap-3 border-t border-base-content/10 pt-5 sm:grid-cols-2"
                      >
                          ...
                          <article
                            ...
                            className={`rounded-xl border p-4 ${
                              billing.plan === plan.key
                                ...
                                : "border-base-content/10 bg-base-content/[0.025]"
                            }`}
                          >
                    ...
                    {billing.canManageBilling && showBillingProfile ? (
                      <form
                        className="mt-5 rounded-lg border border-base-content/10 bg-base-content/[0.025] p-4"
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-1 text-xs leading-5 text-base-content/55">
                              Self-declared details used for Checkout, invoices, and tax handling. This is not an
                              external identity or company verification.
                            </p>
                          </div>
                          {billingProfileSaved ? (
                        ...
                        <div className="mt-4 grid gap-3 sm:grid-cols-2">
                          ...
                          <TextareaField
                            ...
                            className="min-h-20 rounded-lg border-base-content/10 bg-[var(--rateloop-field)]"
                          ...
                          <Field
                            label="Country"
                            className="rounded-lg border-base-content/10 bg-[var(--rateloop-field)] uppercase"
              ...
              {selected && showPanelFunding ? (
                ...
                >
                  ...
                  <div className="mt-5 grid gap-3 border-t border-base-content/10 pt-4 text-center sm:grid-cols-3">
                    <div>
                      <span className="block text-lg font-semibold">
                        ${usdc(selected.prepaid.settledAtomic, locale)}
                      </span>
                    ...
                    <div>
                      <span className="block text-lg font-semibold">
                        ${usdc(selected.prepaid.reservedAtomic, locale)}
                      </span>
                      <span className="inline-flex items-center gap-1 text-xs text-base-content/55">
                        Reserved USDC
                    ...
                    <div>
                      <span className="block text-lg font-semibold">
                        ${usdc(selected.prepaid.availableAtomic, locale)}
                      </span>
                      <span className="inline-flex items-center gap-1 text-xs text-base-content/55">
                        Available USDC
                  ...
                  {canManageTopups && topups?.enabled ? (
                    <form
                      className="mt-5 flex flex-col gap-3 border-t border-base-content/10 pt-4 sm:flex-row"
                  ...
                  {topups?.ledger.length || topups?.reservations.length ? (
                    <details className="mt-4 rounded-lg border border-base-content/10 p-3">
              ...
              {selected && canManageIdentity && identity?.enabled ? (
                <section
                  aria-labelledby="enterprise-identity"
                  className="mt-5 rounded-xl border border-base-content/10 p-5"
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <>
                          {identityToken ? (
                            <div className="rounded-lg border border-warning/25 bg-warning/[0.06] p-3">
                              ...
                              <code className="mt-2 block break-all rounded bg-[var(--rateloop-code-surface)] p-2 text-xs">
                                {identityToken}
                              </code>
                              {identityEndpoint ? (
                                <>
                            ...
                            <div className="mt-3 grid gap-3 sm:grid-cols-2">
                              ...
                              {identityForm.protocol === "oidc" ? (
                                ...
                                <>
                                  ...
                                  <TextareaField
                                    containerClassName="sm:col-span-2"
                                    className="min-h-24 rounded-lg border-base-content/10 bg-[var(--rateloop-field)] font-mono text-xs"
