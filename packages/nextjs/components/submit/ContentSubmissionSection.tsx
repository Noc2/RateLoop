...
const MIN_HUMAN_RESPONSE_WINDOW_SECONDS = 20 * SECONDS_PER_MINUTE;
...
const ROUND_RESPONSE_WINDOW_PRESETS = [
  { id: "2m", label: "2 min", seconds: 2 * SECONDS_PER_MINUTE },
  { id: "5m", label: "5 min", seconds: 5 * SECONDS_PER_MINUTE },
  { id: "20m", label: "20 min", seconds: 20 * SECONDS_PER_MINUTE },
  { id: "1h", label: "1 hour", seconds: SECONDS_PER_HOUR },
  { id: "24h", label: "1 day", seconds: 24 * SECONDS_PER_HOUR },
  { id: "3d", label: "3 days", seconds: 3 * 24 * SECONDS_PER_HOUR },
  { id: "7d", label: "7 days", seconds: 7 * 24 * SECONDS_PER_HOUR },
  { id: "14d", label: "14 days", seconds: 14 * 24 * SECONDS_PER_HOUR },
  { id: "30d", label: "30 days", seconds: 30 * 24 * SECONDS_PER_HOUR },
] as const;
...
function formatShortAddress(address: string | null | undefined): string {
...
export function ContentSubmissionSection() {
  ...
  const roundBlindSecondBounds = useMemo(() => {
    const min = roundConfigBounds.minEpochDuration;
    const max = Math.max(min, roundConfigBounds.maxEpochDuration);
    return { min, max };
  ...
  const hasShortHumanResponseWindow =
    parsedRoundBlindSeconds > 0 && parsedRoundBlindSeconds < MIN_HUMAN_RESPONSE_WINDOW_SECONDS;
  ...
  const estimatedFeedbackBonusRecipientAmount =
    feedbackBonusMode === "enabled" && selectedFeedbackBonusAmount
      ...
      : 0n;
  const feedbackBonusWindowLabel = estimatedBountyStartByLabel;
      ...
      if (
        ...
        try {
          ...
          feedbackBonusFundingError =
            getTransactionErrorMessage(feedbackBonusError) || "Feedback Bonus funding transaction failed.";
        }
          ...
          <div className="grid grid-cols-3 gap-2">
            {ROUND_RESPONSE_WINDOW_PRESETS.map(option => (
              <button
                ...
                onClick={() => updateRoundBlindSecondsInput(String(option.seconds))}
          ...
          <div className="surface-card-nested rounded-lg p-4">
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <p className="flex items-center gap-1.5 text-base text-base-content/70">
                  Feedback close
                  <InfoTooltip text="Feedback Bonus submissions use the same close as the question duration." />
                </p>
        ...
        <div className="space-y-4">
          ...
          <div>
            <p className="flex items-center gap-1.5 text-base text-base-content/70">
              ...
              <InfoTooltip text="The effective award decision deadline is at least 1 hour after the round settles, even when this requested close is earlier." />
            </p>
            <p className="mt-1 text-base font-medium text-base-content">{feedbackBonusWindowLabel}</p>
          </div>
        </div>
