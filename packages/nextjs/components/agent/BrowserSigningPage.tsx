...
export function BrowserSigningPage({ intentId }: { intentId: string }) {
  ...
  const handleExecute = useCallback(async () => {
    ...
    try {
      const transactionPlan = preparedIntent.transactionPlan;
      ...
      if (preparedIntent.chainId && chain?.id !== preparedIntent.chainId) {
        await switchToChain(preparedIntent.chainId);
      }
      ...
      const intentChainId = preparedIntent.chainId ?? undefined;
      const hashes = await executeWalletTransactionPlan({
        action: "browser signing ask submission",
        calls,
    ...
    <AppPageShell contentClassName="space-y-5" paddingTopClassName="pt-6">
      <section className="surface-card rounded-lg p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-base-content/50">Agent signing link</p>
      ...
      {isLoading ? (
        <div className="surface-card rounded-lg p-6">
          <span className="loading loading-spinner loading-sm text-primary" /> Loading signing link...
        </div>
      ) : null}
          ...
          <section className="surface-card rounded-lg p-5">
            ...
            {intent.transactionPlan?.calls?.length ? (
              ...
              <p className="mt-4 text-sm text-base-content/60">
                Prepare this signing link to fetch the wallet calls. x402 asks may first request a typed-data signature,
                then return the transaction plan.
              </p>
            )}
          </section>
        </>
