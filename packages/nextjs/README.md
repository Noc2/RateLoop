...
| `CURYO_MCP_ALLOW_UNLIMITED_BUDGET`                | Static `CURYO_MCP_BEARER_TOKEN` ask tokens must set positive daily and per-ask budgets unless this explicit opt-in is `true`; JSON `CURYO_MCP_AGENTS` budgets are configured per entry |
