...
- Native EIP-3009/x402 asks no longer need a reservation transaction. They remain USDC-only: bounty-only asks return one submit call after the USDC authorization is signed, and single-question asks with a USDC Feedback Bonus use `submitQuestionWithX402OneShotPayment` so bounty and bonus funding land in the same transaction. Wallet-call asks can use LREP or USDC bounties plus same-asset Feedback Bonuses; bonus funding is prepared and confirmed as a follow-up plan after the question submission is confirmed.
