...
| `NEXT_PUBLIC_USE_BASE_PRECONF_RPC`                     | Set to `true` on deployed frontend environments to use Base Flashblocks/preconfirmation RPC metadata for browser transaction UX                                                              |
...
- Base frontend targets can use Flashblocks/preconfirmation chain metadata and poll preconfirmation RPCs at a 500ms interval for faster wallet progress. Keep Ponder and Keeper on sealed-block RPCs; the preconfirmation RPCs are for user-facing transaction UX, not canonical indexing.
