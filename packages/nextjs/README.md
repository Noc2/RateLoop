...
| `NEXT_PUBLIC_ALLOW_UNDEPLOYED_TARGET_NETWORKS`    | Bootstrap-only flag for Vercel/testnet builds before generated `deployedContracts` metadata exists; unset once contracts are deployed                                                   |
...
- No core contract address env vars are needed for supported chains. The frontend reads core deployment metadata from `@rateloop/contracts` and fails fast if `NEXT_PUBLIC_TARGET_NETWORKS` includes a chain without those definitions, except for the temporary `NEXT_PUBLIC_ALLOW_UNDEPLOYED_TARGET_NETWORKS=true` bootstrap path used before testnet contract metadata exists. Rollout contracts such as question reward pools can use their documented env fallbacks until generated metadata catches up.
