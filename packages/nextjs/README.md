...
- The Ask page and browser agent handoff can host one TXT or Markdown context document per question. Uploaded documents are UTF-8 normalized, moderated with OpenAI, stored only as approved text, and rendered publicly at `/context/documents/{documentId}` for use as the question `contextUrl`. PDF uploads are intentionally deferred.
