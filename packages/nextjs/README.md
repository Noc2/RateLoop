...
| `NEXT_PUBLIC_WORLD_ID_PROOF_MODE`                 | World ID proof migration mode: `legacy` (default), `compat`, or `v4`                                                                                                                         |
...
- `NEXT_PUBLIC_WORLD_ID_PROOF_MODE=legacy` preserves the current World ID 3.0 flow. `compat` requests v4-compatible proofs while allowing legacy fallback, and `v4` rejects legacy proofs. The UI only submits a v4 proof when the configured mode allows it and the deployed `RaterRegistry` ABI exposes `attestHumanCredentialWithV4Proof`.
