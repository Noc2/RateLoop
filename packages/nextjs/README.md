...
- Native EIP-3009/x402 asks no longer need a reservation transaction. They remain USDC-only: bounty-only asks return one submit call after the USDC authorization is signed, and single-question asks with a USDC Feedback Bonus use `submitQuestionWithX402OneShotPayment` so bounty and bonus funding land in the same transaction. Wallet-call asks can use LREP or USDC bounties and creation-time LREP or USDC Feedback Bonuses; separate post-confirmation bonus funding is disabled for the fresh redeploy.
