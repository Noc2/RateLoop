...
| `NEXT_PUBLIC_RPC_URL_8453`                             | Optional browser RPC override for Base mainnet; required for that chain when Base preconfirmation is enabled and should point at a Flashblocks-capable provider                                                                                                                                           |
| `NEXT_PUBLIC_RPC_URL_84532`                            | Optional browser RPC override for the isolated Base Sepolia tokenless deployment                                                                                                                                                                                                                          |
| `NEXT_PUBLIC_USE_BASE_PRECONF_RPC`                     | Set to `true` on deployed Vercel/Railway environments to use Base Flashblocks/preconfirmation chain metadata with the configured `NEXT_PUBLIC_RPC_URL_<chainId>`                                                                                                                                          |
| `RATELOOP_SERVER_USE_BASE_PRECONF_RPC`                 | Optional server-only opt-in for using Base preconfirmation metadata with the same configured `NEXT_PUBLIC_RPC_URL_<chainId>` during server confirmation; leave false unless vetted                                                                                                                        |
...
| `NEXT_PUBLIC_TARGET_NETWORKS`                          | Comma-separated deployed chain IDs exposed in the UI; supported targets are local Foundry (`31337`), Base mainnet (`8453`), and Base Sepolia (`84532`)                                                                                                                                                    |
...
| `NOTIFICATION_DELIVERY_SECRET`                         | Secret for notification email unsubscribe links and email delivery execution; scheduled Vercel GET delivery can authenticate with `CRON_SECRET`, but this secret must also be set                                                                                                                         |
| `NEXT_PUBLIC_PONDER_URL`                               | Public Ponder indexer URL (required in production)                                                                                                                                                                                                                                                        |
| `PONDER_METADATA_SYNC_TOKEN`                           | Server-side bearer token for late question metadata sync to Ponder; must match the Ponder service token                                                                                                                                                                                                   |
| `THIRDWEB_SERVER_VERIFIER_SECRET`                      | Shared secret used by the thirdweb server verifier webhook                                                                                                                                                                                                                                                |
| `RATELOOP_X402_USDC_ADDRESS`                           | Optional USDC override for direct agent bounty planning; prefer chain-scoped `RATELOOP_X402_USDC_ADDRESS_<chainId>` for live multi-chain configs                                                                                                                                                          |
...
| `NEXT_PUBLIC_USDC_ADDRESS`                             | Optional browser-side USDC override for USDC bounties; prefer chain-scoped `NEXT_PUBLIC_USDC_ADDRESS_<chainId>`                                                                                                                                                                                           |
...
| `RATELOOP_E2E_PRODUCTION_BUILD`                        | Server-side opt-in for local production-style E2E builds; rejected for production deploys, mainnet target networks, or non-local app URLs                                                                                                                                                                 |
| `NEXT_PUBLIC_RATELOOP_E2E_PRODUCTION_BUILD`            | Browser-side opt-in for local production-style E2E builds; rejected for production deploys, mainnet target networks, or non-local app URLs                                                                                                                                                                |
...
| `RATELOOP_IMAGE_ATTACHMENT_SWEEP_SECRET`               | Bearer/header secret for the image attachment sweep route that deletes stale unattached uploaded images                                                                                                                                                                                                   |
...
| `CRON_SECRET`                                          | Vercel Cron bearer secret accepted by attachment cleanup, notification email delivery, confidentiality disclosure reconciliation, log-root publication, and callback sweep jobs; for notification delivery this only authenticates the cron request, and `NOTIFICATION_DELIVERY_SECRET` is still required |
...
- Browser RPC reads prefer `NEXT_PUBLIC_RPC_URL_<chainId>` overrides first, then `NEXT_PUBLIC_ALCHEMY_API_KEY`, then the chain's default public RPC list. When `NEXT_PUBLIC_USE_BASE_PRECONF_RPC=true`, Base targets require the matching `NEXT_PUBLIC_RPC_URL_<chainId>` and use that configured RPC for preconfirmation reads; there is no dedicated preconfirmation env var or Base public preconfirmation fallback.
- The target-network parser accepts local Foundry, Base mainnet, and Base Sepolia: `31337`, `8453`, and `84532`. Live builds fail fast if the selected chain does not have generated deployment metadata in `@rateloop/contracts`. Unversioned bundles retain the legacy requirements; tokenless bundles must declare `__rateloopDeployment.schemaVersion = "tokenless-v1"` and contain only `TokenlessPanel`, `CredentialIssuer`, `USDC`, and optional `X402PanelSubmitter` entries.
- Base frontend targets can use Flashblocks/preconfirmation chain metadata and poll active preconfirmation receipt status at a 200ms interval for faster wallet progress. Next.js server confirmation ignores the public browser toggle by default; only set `RATELOOP_SERVER_USE_BASE_PRECONF_RPC=true` with a provider whose receipt APIs are acceptable for server confirmation. Keep Ponder and Keeper on sealed-block RPCs; preconfirmation RPCs are for user-facing transaction UX, not canonical indexing.
- The Wallet settings tab uses thirdweb's BuyWidget to add native ETH for the configured live target network. Configure the thirdweb client ID's allowed domains for the production and preview origins that will render `/settings#wallet`.
...
- Native EIP-3009/x402 asks no longer need a reservation transaction. They remain USDC-only: bounty-only asks return one submit call after the USDC authorization is signed, and single-question asks with a USDC Feedback Bonus use `submitQuestionWithX402OneShotPayment` so bounty and bonus funding land in the same transaction. Wallet-call asks can use LREP or USDC bounties plus LREP or USDC Feedback Bonuses; bonus funding is prepared and confirmed as a follow-up plan after the question submission is confirmed.
...
- For local development, `yarn dev:db` and `yarn dev:stack` manage a Docker Postgres container when `DATABASE_URL` points to localhost. `yarn dev:stack` only runs `db:push` automatically for local databases. For non-local databases, apply numbered SQL migrations through the deployment database migration process; `yarn workspace @rateloop/nextjs db:push` and `yarn dev:stack --allow-remote-db-push` are controlled schema-sync tools, not the deploy migration runner.
