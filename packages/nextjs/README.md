...
| `NEXT_PUBLIC_USE_BASE_PRECONF_RPC`                     | Set to `true` on deployed Vercel/Railway environments to use Base Flashblocks/preconfirmation RPC metadata; confirm the provider is acceptable for server receipt reads before enabling       |
...
- Base frontend targets can use Flashblocks/preconfirmation chain metadata and poll preconfirmation RPCs at a 500ms interval for faster wallet progress. Because the toggle is public, Next.js server confirmation code can also resolve that RPC metadata; enable it only with a provider whose receipt APIs are acceptable for server confirmation. Keep Ponder and Keeper on sealed-block RPCs; the preconfirmation RPCs are for user-facing transaction UX, not canonical indexing.
