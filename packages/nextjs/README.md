...
| `NEXT_PUBLIC_RPC_URL_84532`                            | Optional generic browser RPC override for Base Sepolia                                                                                                                                       |
| `NEXT_PUBLIC_RPC_URL_8453`                             | Optional generic browser RPC override for Base mainnet                                                                                                                                       |
| `NEXT_PUBLIC_USE_BASE_PRECONF_RPC`                     | Set to `true` on deployed Vercel/Railway environments to use Base Flashblocks/preconfirmation RPC metadata for browser wallet progress                                                       |
| `NEXT_PUBLIC_BASE_PRECONF_RPC_URL_84532`               | Optional dedicated Flashblocks/preconfirmation RPC override for Base Sepolia; preferred over generic RPC overrides when preconfirmation RPC is enabled                                       |
| `NEXT_PUBLIC_BASE_PRECONF_RPC_URL_8453`                | Optional dedicated Flashblocks/preconfirmation RPC override for Base mainnet; preferred over generic RPC overrides when preconfirmation RPC is enabled                                       |
| `RATELOOP_SERVER_USE_BASE_PRECONF_RPC`                 | Optional server-only opt-in for using Base preconfirmation RPC metadata during server confirmation; leave false unless the provider is vetted for receipt reads                              |
| `RATELOOP_SERVER_BASE_PRECONF_RPC_URL_84532`           | Optional dedicated Base Sepolia server preconfirmation RPC override when `RATELOOP_SERVER_USE_BASE_PRECONF_RPC=true`                                                                         |
| `RATELOOP_SERVER_BASE_PRECONF_RPC_URL_8453`            | Optional dedicated Base mainnet server preconfirmation RPC override when `RATELOOP_SERVER_USE_BASE_PRECONF_RPC=true`                                                                         |
...
- Browser RPC reads prefer `NEXT_PUBLIC_RPC_URL_<chainId>` overrides first, then `NEXT_PUBLIC_ALCHEMY_API_KEY`, then the chain's default public RPC list. When `NEXT_PUBLIC_USE_BASE_PRECONF_RPC=true`, Base targets instead prefer `NEXT_PUBLIC_BASE_PRECONF_RPC_URL_<chainId>`, then generic browser RPC overrides, Alchemy, Base's standard public RPCs, and finally Base's public preconfirmation RPC as a last fallback.
...
- Base frontend targets can use Flashblocks/preconfirmation chain metadata and poll preconfirmation RPCs at a 2s interval for faster wallet progress. Ordinary Base RPCs poll at 5s, still faster than the global 30s UI interval. Next.js server confirmation ignores the public browser toggle by default; only set `RATELOOP_SERVER_USE_BASE_PRECONF_RPC=true` with a provider whose receipt APIs are acceptable for server confirmation. Keep Ponder and Keeper on sealed-block RPCs; the preconfirmation RPCs are for user-facing transaction UX, not canonical indexing.
...
- Native EIP-3009/x402 asks no longer need a reservation transaction. They remain USDC-only: bounty-only asks return one submit call after the USDC authorization is signed, and single-question asks with a USDC Feedback Bonus use `submitQuestionWithX402OneShotPayment` so bounty and bonus funding land in the same transaction. Wallet-call asks can use LREP or USDC bounties; LREP Feedback Bonuses still use the separate post-confirmation wallet-call plan.
