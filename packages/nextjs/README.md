...
- Native EIP-3009/x402 asks no longer need a reservation transaction. Bounty-only asks return one submit call after the USDC authorization is signed; single-question asks with a USDC Feedback Bonus use `submitQuestionWithX402OneShotPayment` so bounty and bonus funding land in the same transaction. LREP Feedback Bonuses still use the separate post-confirmation wallet-call plan.
