...
| `NEXT_PUBLIC_WORLD_ID_CREDENTIAL_ACTION`          | Optional World ID v4 credential action ID; defaults to `rateloop-human-credential-v1`                                                                                                        |
| `NEXT_PUBLIC_WORLD_ID_PRESENCE_ACTION`            | Optional World ID v4 fresh recheck action ID; defaults to `rateloop-human-presence-v1`                                                                                                       |
| `NEXT_PUBLIC_WORLD_ID_ENVIRONMENT`                | World ID environment, `production` or `staging`                                                                                                                                              |
| `NEXT_PUBLIC_WORLD_ID_ENABLE_V4_SELFIE`           | Set to `true` only after the deployed World app has confirmed World ID v4 Selfie/Face support; hidden by default                                                                             |
| `WORLD_ID_V4_IDKIT_RP_ID`                         | Required for World ID requests; the public IDKit relying-party ID from the World Developer Portal (`rp_...`), used in `rp_context.rp_id`                                                     |
| `WORLD_ID_V4_RP_ID`                               | Legacy fallback for Next.js only when it contains an `rp_...` ID; Foundry deploys use this name for the numeric on-chain verifier RP ID, so prefer `WORLD_ID_V4_IDKIT_RP_ID` in app runtime |
...
| `RATELOOP_MCP_AUTHORIZATION_SERVER_URL`           | Optional real OAuth/OIDC authorization server advertised in MCP protected-resource metadata; omit for pre-registered bearer-token agents                                                     |
...
- The target-network parser accepts local Foundry plus World Chain Sepolia and mainnet: `31337`, `4801`, and `480`. Production builds use the committed World Chain Sepolia default in `.env.production` unless Vercel/system env config overrides `NEXT_PUBLIC_TARGET_NETWORKS`.
...
- The Identity settings tab uses World ID v4 to produce a wallet-bound proof. The connected wallet submits that proof directly to `RaterRegistry.attestHumanCredentialWithV4Proof`, where the World ID verifier checks it on-chain before any credential exists. There is no server issuer wallet in the normal path.
- Bounties can stay open to everyone or require any selected mix of Passport, Proof of Human, and Selfie Check credentials. Selfie Check is v4-gated by `NEXT_PUBLIC_WORLD_ID_ENABLE_V4_SELFIE`; any scoped bounty can additionally require a fresh `attestHumanPresenceWithV4Proof` recheck shortly before the vote commit.
...
- Agent clients should follow the AI docs flow: list templates, quote with `walletAddress`, ask with a stable client request ID, execute and confirm wallet calls, wait for a status read or signed managed callback, then fetch the structured result. Operator token lifecycle, scopes, budgets, category allowlists, callback recovery, and audit history belong in `/ask?tab=agent` for managed agents; static `RATELOOP_MCP_AGENTS` remains supported for server-configured policies.
- The Ask page and MCP tools can host JPG, PNG, and WEBP image context through private Vercel Blob uploads. Uploaded images are validated, metadata-stripped into WEBP, moderated with OpenAI, and served through `/api/attachments/images/{attachmentId}.webp` only after approval. Agents that generate or capture image bytes should call `rateloop_upload_image` directly with a managed token, or call `rateloop_prepare_image_upload` plus `rateloop_upload_image` in public wallet mode, instead of asking users to find a third-party image host.
- The Ask page keeps the on-chain `description` field as a short Voter Summary and can host longer Details text off-chain. Approved Details are UTF-8 normalized, moderated with OpenAI, exposed as `text/plain` at `/api/attachments/details/{detailsId}`, and committed on-chain through `detailsUrl` plus `detailsHash` so other frontends can fetch and verify the full text.
