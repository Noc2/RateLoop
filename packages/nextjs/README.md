...
| `NEXT_PUBLIC_RPC_URL_84532`                            | Optional browser RPC override for Base Sepolia; required for that chain when Base preconfirmation is enabled and should point at a Flashblocks-capable provider                              |
...
| `NEXT_PUBLIC_TARGET_NETWORKS`                          | Comma-separated deployed chain IDs exposed in the UI; use Base mainnet (`8453`) for production and Base Sepolia (`84532`) for staging/validation                                             |
...
| `NOTIFICATION_DELIVERY_SECRET`                         | Secret for notification email unsubscribe links and manual delivery POSTs; scheduled Vercel GET delivery can authenticate with `CRON_SECRET`                                                |
...
| `CRON_SECRET`                                          | Vercel Cron bearer secret accepted by attachment cleanup, notification email delivery, confidentiality disclosure reconciliation, log-root publication, and callback sweep jobs              |
...
- The target-network parser accepts local Foundry, Base Sepolia/mainnet, and World Chain Sepolia/mainnet: `31337`, `84532`, `8453`, `4801`, and `480`. Live builds fail fast if the selected chain does not have generated deployment metadata in `@rateloop/contracts`.
