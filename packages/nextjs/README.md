...
| `NEXT_PUBLIC_TARGET_NETWORKS`                          | Comma-separated deployed chain IDs exposed in the UI; use Base mainnet (`8453`) for production and local Foundry (`31337`) for development                                                  |
...
- The target-network parser accepts local Foundry plus the active Base deployment target: `31337` and `8453`. Live builds fail fast if the selected chain does not have generated deployment metadata in `@rateloop/contracts`.
