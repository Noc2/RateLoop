...
import { sweepOrphanedContextDocuments } from "~~/lib/attachments/contextDocuments";
...
export async function POST(request: NextRequest) {
  const secret = process.env.RATELOOP_CONTEXT_DOCUMENT_SWEEP_SECRET?.trim() ?? "";
  if (!secret) {
    return NextResponse.json({ error: "Context document sweep is not configured." }, { status: 503 });
  }

  const limited = await checkRateLimit(request, SWEEP_RATE_LIMIT, { allowOnStoreUnavailable: true });
  if (limited) return limited;

  const token = request.headers.get("x-rateloop-context-document-sweep-secret")?.trim() || readBearerToken(request);
  if (!isAuthorizedSweepRequest(token, secret)) {
    return NextResponse.json({ error: "Unauthorized." }, { status: 401 });
  }

  return NextResponse.json(await sweepOrphanedContextDocuments({ limit: parseLimit(request) }));
}
