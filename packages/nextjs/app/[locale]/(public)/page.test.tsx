...
test("landing page presents the tokenless human-assurance story", async () => {
  ...
  assert.match(html, /\$0/);
  assert.match(html, /25 completed review decisions each calendar month/);
  assert.match(html, /1 active agent/);
  assert.match(html, /Early Access/);
  assert.match(html, /\$29/);
  assert.match(html, /per workspace\/month/);
  assert.match(html, /First 12 months/);
  assert.match(html, /250 completed review decisions each subscription period/);
  assert.match(html, /3 active agents/);
  assert.match(html, /Enterprise/);
  assert.match(html, /Custom volumes and terms/);
  assert.match(html, /invited, unpaid reviewers/);
  assert.equal(html.match(/href="\/pricing"/g)?.length, 1);
  assert.match(html, /Compare plans/);
  assert.ok(html.indexOf("Free") < html.indexOf("Early Access"));
  assert.ok(html.indexOf("Early Access") < html.indexOf("Enterprise"));
