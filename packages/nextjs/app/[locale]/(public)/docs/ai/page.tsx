  ...
  return (
    <LocalizedPublicContent locale={locale} section="docs">
      <article className="prose max-w-none">
        ...
        <p>
          The workspace MCP at <code>{workspaceMcpUrl}</code> binds an approved agent version to its owner-approved
          review policy. A safe connection can read that context and evaluate review requirements, but it cannot spend,
          publish, read private artifacts, or administer the workspace.
        </p>
        <ol>
          <li>
        ...
        <p>
          Host-enforced review is a reserved, planned mode and cannot be enabled. Its activation gate remains
          fail-closed until a separately verified adapter supplies the required enforcement evidence.
        </p>
...
codex plugin add rateloop@rateloop`}</CodeBlock>
        ...
        <details className="not-prose mt-8 rounded-xl border border-base-content/10 bg-base-200/40 p-4">
          ...
          <div className="mt-4 space-y-3 text-sm leading-6 text-base-content/70">
            <p>
              MCP compatibility belongs to the host and agent loop, not the model brand. The model needs tool use, and
              the host must implement remote Streamable HTTP, OAuth, tool results, and the session lifecycle.
            </p>
            <p>
        ...
        <p>
          The public MCP at <code>{remoteMcpUrl}</code> prepares a draft for a browser decision. Creating a handoff is
          not submission, and it does not activate an unavailable reviewer network or paid lane.
        </p>
        ...
        <p>
          ...
          safety, legal, medical, or compliance approval. Use only authorized, minimized material and keep an
