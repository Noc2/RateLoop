...
export function generateMetadata({ params }: { params: PublicLocaleParams }): Promise<Metadata> {
  return getLocalizedPublicMetadata({ params, section: "docs", title: "SDK guide" });
}

export default function SdkPage({ params }: { params?: PublicLocaleParams } = {}) {
    ...
    <LocalizedPublicContent locale={locale} section="docs">
      <article className="prose max-w-none">
        ...
        <p className="lead text-base-content/60 text-lg">
          ...
          review path uses invited, unpaid reviewers through the connected workspace MCP and does not activate this
          flow.
        </p>
...
GET /api/account/workspaces/{workspaceId}/assurance/trusted-keys?format=spki&keyId=ed25519:…`}</code>
        ...
        <p>
          ...
          <Link href="/api/evidence/trusted-keys">verification-key endpoint</Link>. Workspace members can also download
          it from the authenticated key history. Run the local checkers with an explicit key ID instead of trusting keys
          or heads from the same export:
        </p>
        <pre>
