...
export function generateMetadata({ params }: { params: PublicLocaleParams }): Promise<Metadata> {
  return getLocalizedPublicMetadata({ params, section: "docs", title: "How RateLoop works" });
}

export default function HowTokenlessWorksPage({ params }: { params?: PublicLocaleParams } = {}) {
    ...
    <LocalizedPublicContent locale={locale} section="docs">
      <article className="prose max-w-none">
        ...
        <div className="not-prose my-8">
          <HumanAssuranceLoop locale={locale} />
        </div>
        ...
        <p>
          ...
          integrations are advisory; only a verified host integration that controls delivery can establish that the
          output stayed blocked while it waited. No host currently holds that tier.
        </p>
        <p>
        ...
        <p>
          Submit only material you are authorized to share. Minimize or redact personal, confidential, and regulated
          data because assigned reviewers and authorized RateLoop workloads may read it to provide the service.
        </p>
