...
const TECH_STACK_DESCRIPTION =
  "Technical reference for RateLoop's connected review and separately gated settlement architecture.";
...
export default function TokenlessTechStackPage({ params }: { params?: PublicLocaleParams } = {}) {
    ...
    <LocalizedPublicContent locale={locale} section="docs">
      <article className="prose max-w-none">
        ...
        <p className="lead text-base-content/60 text-lg">
          This is a technical architecture reference. The current hosted path uses invited, unpaid workspace reviewers
          and does not activate the fund-backed settlement mechanisms described below.
        </p>

        <TokenlessArchitectureDiagram locale={locale} />
        ...
        <p>
          The agent-funded lane uses an x402-style payment authorization: the agent wallet signs short-lived EIP-3009
          USDC terms, then <code>X402PanelSubmitter</code> consumes that authorization and funds the selected panel on
          Base. The private key stays with the agent. If the nonce is already used, RateLoop accepts only an exact
          matching round receipt; otherwise the payment is marked possibly paid and no replacement authorization is
          retried. A prepaid workspace balance provides the wallet-free lane.
        </p>
        ...
        <p>
          Every ask freezes one content-hashed audience policy for its selected reviewer source. The policy binds
          reviewer source, eligibility, panel size, economics, and publication rules so the audience cannot change
          midway through a round. The schema reserves separate hybrid subpanels, but that lane is unavailable until both
          children have durable release, terminal, expiry, and refund processing.
        </p>

        <h2 id="correlation-diversified-assignment">Correlation-Diversified Assignment</h2>
        <p>
          Signed correlation epochs help avoid repeatedly assigning closely connected reviewers to the same panel.
          Private linkage features stay encrypted; result evidence carries only the frozen epoch reference and aggregate
          coverage. Post-round analytics can shape publication and future eligibility while accepted work keeps its
          promised pay.
        </p>

        <h2 id="commit-reveal">Commit-Reveal</h2>
        <p>
          A report contains a binary answer, a prediction of the panel&apos;s answer share, and an optional reason.
          During the blind phase the reviewer submits a commitment to the sealed report. Reveal verifies that the opened
          report matches the accepted commitment, so early reviewers cannot copy visible momentum.
        </p>

        <h2 id="drand-tlock">drand/tlock</h2>
        <p>
          Reports are sealed to a future drand beacon round with timelock encryption. When that public randomness
          arrives, the keeper can open reports without holding a universal decryption key; reviewers retain a
          self-reveal path. The round then uses only valid opened reports for settlement.
        </p>
        ...
        <p>
          ...
          the share of reviewers who will answer up. A canonical peer supplies the comparison answer and another
          supplies the reference prediction. The scoring version is fixed in the round evidence and uses integer
        ...
        <RbtsPayoutDiagram locale={locale} />
        ...
        />
        <SurprisinglyPopularBonusChart locale={locale} />
        ...
        <p>
          The application joins the public settlement record with customer-scoped context: reports, reasons,
          disagreement, reviewer source, scoring inputs, payment state, and evidence hashes. Agents receive a structured
          result; humans get the same evidence in a readable decision view.
        </p>
  ...
  return (
    <LocalizedPublicContent locale={locale} section="docs">
      <Card as="section" variant="marketing" className="rounded-2xl p-5 sm:p-6">
        <p className="font-mono text-xs font-semibold uppercase tracking-[0.16em] text-[var(--rateloop-blue)]">
          {label}
        </p>
