...
const CONNECT_HOST_DESCRIPTION = "Per-host RateLoop connection guides generated from the host capability registry.";

export function generateMetadata({ params }: { params: PublicLocaleParams }) {
  return getLocalizedPublicMetadata({
    params,
    section: "docs",
    title: "Connect a Host",
    description: CONNECT_HOST_DESCRIPTION,
  });
}
...
function HostList({ hosts, locale }: { hosts: readonly TokenlessHostCapability[]; locale: "en" | "de" }) {
  return (
...
export default function ConnectHostIndexPage({ params }: { params?: PublicLocaleParams } = {}) {
    ...
    <LocalizedPublicContent locale={locale} section="docs">
      <article className="prose max-w-none">
        ...
        <p className="lead text-base-content/60 text-lg">
          Copy the connection message from <Link href="/agents/connections">Connections</Link>, then open your
        ...
        <h2>Primary path</h2>
        <HostList hosts={PRIMARY_HOSTS} locale={locale} />

        <h2>Other hosts</h2>
        {OTHER_CATEGORIES.map(category => (
          <section key={category}>
            <h3>{HOST_CATEGORY_LABELS[category]}</h3>
            <HostList hosts={OTHER_HOSTS.filter(host => host.category === category)} locale={locale} />
          </section>
        ))}
