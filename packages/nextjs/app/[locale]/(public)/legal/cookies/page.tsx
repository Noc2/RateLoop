...
export function generateMetadata({ params }: { params: PublicLocaleParams }): Promise<Metadata> {
  return getLocalizedPublicMetadata({ params, section: "legal", title: "Cookies and browser storage" });
}

export default function CookiesPage({ params }: { params?: PublicLocaleParams } = {}) {
  ...
  return (
    <LocalizedPublicContent locale={locale} section="legal">
      <article className="prose legal-prose mx-auto max-w-4xl px-4 py-12">
        ...
        <ul>
          <li>
            <strong>__Host-rateloop-session</strong> (or <strong>rateloop-session</strong> in local development): a
            random RateLoop application-session credential. The server stores only its hash. It is HttpOnly, same-site,
            and expires after up to 12 hours or earlier on logout/revocation.
          </li>
          <li>
        ...
        <ul>
          ...
          <li>
            <strong>Optional device recovery:</strong> when a reviewer creates a recovery package, local storage keeps
            the encrypted recovery record on that device until the user removes it or clears site data. RateLoop cannot
            restore browser storage that the user deletes.
          </li>
          <li>
            <strong>Integration choice:</strong> local storage may remember the agent host selected for a workspace. It
            is a convenience preference and can be removed by resetting the choice or clearing site data.
          </li>
          <li>
        ...
        <p>
          ...
          privacy-enhanced player only after the user chooses play. Google may then receive network and device
          information and may place or read storage under its own policy. Playing the video is optional.
        </p>
