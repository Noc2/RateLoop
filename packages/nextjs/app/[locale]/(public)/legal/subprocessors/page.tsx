...
export function generateMetadata({ params }: { params: PublicLocaleParams }): Promise<Metadata> {
  return getLocalizedPublicMetadata({ params, section: "legal", title: "Subprocessors" });
}
  ...
  return (
    <LocalizedPublicContent locale={locale} section="legal">
      <article className="prose legal-prose mx-auto max-w-5xl px-4 py-12">
        ...
        <p>
          This page identifies providers RateLoop is authorized to use when it processes Customer Personal Data under
          the <Link href="/legal/dpa">Data Processing Addendum</Link>. A conditional provider receives covered data only
          when its corresponding feature is enabled and used. Customers may request a snapshot of providers enabled for
          their workspace and the applicable transfer mechanism.
        </p>

        <h2>Core hosted subprocessors</h2>
        <ProviderTable providers={coreProviders} locale={locale} />

        <h2>Conditional feature subprocessors</h2>
        <ProviderTable providers={conditionalProviders} locale={locale} />

        <h2>Services that may be independent recipients</h2>
        <p>
          A public Base transaction, USDC transfer, or drand/timelock action is distributed through public
          infrastructure and is not a confidential processor copy. World ID or another eligibility provider, a
          self-custodial wallet provider, and the privacy-enhanced YouTube player may act as an independent controller
          for their own purposes under their terms. RateLoop contacts the YouTube player only after the user chooses
          play. The product discloses these boundaries before the relevant optional action.
        </p>
