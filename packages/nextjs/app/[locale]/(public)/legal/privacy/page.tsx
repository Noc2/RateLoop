...
export function generateMetadata({ params }: { params: PublicLocaleParams }): Promise<Metadata> {
  return getLocalizedPublicMetadata({ params, section: "legal", title: "Privacy notice" });
}

export default function PrivacyPage({ params }: { params?: PublicLocaleParams } = {}) {
      ...
      <article className="prose legal-prose mx-auto max-w-4xl px-4 py-12">
        ...
        >
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-base-content/70">
            ...
            <li>
              Private artifacts are encrypted, but authorized RateLoop workloads can decrypt them to provide the
              service.
            </li>
        ...
        <p>
          ...
          before private object storage; database rows keep opaque object references and tenant-scoped metadata.
          Assigned reviewers receive short-lived leases only for their blinded cases. The site does not use advertising
        ...
        <p>
          ...
          processes an approved draft to create a browser handoff but does not store that draft. The complete draft and
          a high-entropy bearer token are placed in the URL fragment, which browsers do not send in normal HTTP
          requests. The reviewed question and panel terms are stored when the user requests an exact quote; submitting
          the ask is a separate explicit action. Anyone who receives the complete handoff URL may be able to inspect the
          draft and later read its status or result, so users must not share it or include secrets, credentials,
          regulated personal data, or confidential customer material.
        </p>
        <h2 id="browser-sign-in">Browser sign-in</h2>
        <p>
          Browser users sign in through RateLoop&apos;s self-hosted Better Auth service using an email one-time code or
          a registered passkey. Google and Apple are offered only when their credentials are configured. Better Auth and
          the selected email or social provider process the authentication data needed for that sign-in. RateLoop maps
          the provider subject to an opaque RateLoop principal that is independent of any wallet, then exchanges the
          short Better Auth session for its own hashed, time-limited, HttpOnly application session. Ordinary account,
          workspace, invited unpaid review, and API-key agent use do not create or require a wallet. An email address or
          domain alone never grants workspace membership or project access.
        </p>
        <h2 id="optional-wallets">Optional wallets</h2>
        <p>
          A signed-in user may explicitly connect an existing self-custodial wallet for public USDC funding or payout.
          Managed app-wallet creation and recovery through thirdweb are disabled in the hosted service. The connected
          wallet must sign a one-time proof bound to the RateLoop domain, principal, configured Base chain, wallet
          address, selected purpose, nonce, and expiry. Each binding is revocable and never authorizes general account
          or workspace access. Users remain responsible for their wallet keys, provider, and recovery method.
        </p>
        ...
        <p>
          ...
          allocations. RateLoop does not store full card details. Subscription records remain separate from prepaid
          USDC, public-panel funding, participant payout, and settlement records.
        </p>
        ...
        <p>
          ...
          the provider&apos;s own booking page in a new tab, identified by the destination address, and everything
          entered there is collected by that provider under its own privacy terms rather than by RateLoop. That
          typically includes a name, an email address, any answers to the booking questions, and the chosen time; the
          provider may process it outside the European Economic Area. No RateLoop account is created or required, and
          the booking page is reachable by anyone holding the link. RateLoop receives the resulting appointment and its
          contact details as a calendar entry and email, and processes them under its legitimate interest in responding
          to a commercial enquiry. Writing instead to the enterprise contact address avoids the scheduling provider
          entirely.
        </p>
        <h2 id="on-chain-data">On-chain data</h2>
        <p>
          Public-chain interactions publish transaction addresses, commitments, round terms, settlement data, claims,
          and each paid commit&apos;s timelock ciphertext. That ciphertext contains the vote, prediction, response hash,
          ...
          payout address can link paid activity across rounds even though the RateLoop account principal itself is
          opaque.
        </p>
        <p>
          RateLoop presents the public-record disclosure before a reviewer can create a recovery backup or request a
          paid commit. The blockchain assessment follows data-protection-by-design principles and considers the European
          Data Protection Board&apos;s final{" "}
          ...
          blockchain-specific DPIA and a current provider, subprocessor, and international-transfer inventory. Until
          both release gates are complete, RateLoop does not claim launch-level GDPR compliance. Publishing that unmet
          gate is the control: customers can test the release posture against a dated requirement instead of relying on
        ...
        <p>
          ...
          binds the exact assignment policy through current capability, minimum-age, sanctions, tax/DAC7, payout,
          voucher, settlement, and receipt checks. Declared residence, tax residence, document issuer, nationality, and
          any verified residence predicate remain separate fields. Provider evidence, statutory tax records, customer
          ...
          reviewer, voucher, vote key, and nullifier for eligibility, abuse control, and payment operations; RateLoop
          does not claim database-level anonymity.
        </p>
        <p>
        ...
        <p>
          ...
          invited-review history is not used for network admission. This processing is based on RateLoop&apos;s
          legitimate interests in abuse prevention, service security, and reliable reviewer assignment. It never reduces
          or withholds pay already earned. A hard aggregate finding can pause new assignments; the reviewer dashboard
          shows the applicable reason codes and counters. A reviewer can open an appeal, and the assignment consequence
          is suspended while that appeal is open.
        </p>
        <h2 id="recipients-transfers">Recipients, processors, and international transfers</h2>
        <p>
          RateLoop uses service providers for hosting, database/runtime operations, email, billing, and optional
          identity or wallet features. The current categories, named providers, feature conditions, and change-notice
          process are listed on the <Link href="/legal/subprocessors">subprocessor page</Link>. Where covered data is
          transferred outside the EEA, RateLoop uses an applicable adequacy decision or contractual transfer safeguards
          and supplementary measures as required. Public blockchain publication is a separate, user-visible replication
        ...
        <p>
          ...
          cancellation does not override an agreed evidence-retention setting or erase records required for audit,
          dispute handling, accounting, legal holds, or statutory retention. A workspace owner can delete a workspace in
          the product once its funds, subscription, open work, and unsettled billing obligations are resolved. A
          signed-in user can delete their account after deleting owned workspaces, resolving accepted work, and
          deactivating any managed wallet. Deletion revokes access and erases or anonymizes eligible off-chain account
          and workspace data; retained categories remain subject to their applicable purpose and schedule.
        </p>
        <p>
          ...
          erased with account or workspace deletion. A later sign-up starts a new account. Backup copies expire under
          the applicable backup schedule, and public blockchain records remain outside the operator&apos;s ability to
          erase.
        </p>
        <p>
          You may request access, correction, deletion where available, restriction, portability, or object to
          processing, and may withdraw consent for future processing where consent is the basis. Signed-in access and
          export requests enter an authenticated queue; completed exports are available only to the requesting principal
          for seven days. Other requests and questions may be sent to the controller address above. RateLoop normally
          responds within one month, subject to lawful extensions, identity verification, legal holds, and retention
          duties. You may complain to the{" "}
