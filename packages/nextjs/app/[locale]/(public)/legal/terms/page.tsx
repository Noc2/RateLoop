...
export function generateMetadata({ params }: { params: PublicLocaleParams }): Promise<Metadata> {
  return getLocalizedPublicMetadata({ params, section: "legal", title: "Terms" });
}

export default function TermsPage({ params }: { params?: PublicLocaleParams } = {}) {
    ...
    <LocalizedPublicContent locale={locale} section="legal">
      <article className="prose legal-prose mx-auto max-w-4xl px-4 py-12">
        ...
        <p>
          ...
          subscriptions are available only to approved business customers whose legal identity RateLoop has
          independently verified. Paid assignments require an exact frozen assignment policy and all applicable
          capability, tax, sanctions, payout, voucher, settlement, and receipt checks.
        </p>
        <h2 id="customer-material">Customer material and reviewers</h2>
        <p>
          Customers must have the right to submit evaluation material, minimize and redact personal or confidential
          data, choose appropriate reviewers, and provide any required notices. A one-time invitation proves project
          access, not unique humanity, expertise, legal residence, or paid-work eligibility. Customer-invited and
          RateLoop-network results are labeled separately. Hybrid review is not offered in this release.
        </p>
        <p>
        ...
        <p>
          This section applies only where a signed order identifies the customer as the provider of a high-risk AI
          system and identifies RateLoop services that are used or integrated in that system for purposes of Article
          25(4) of the EU AI Act. It does not apply merely because RateLoop operates alongside a customer&apos;s system.
        </p>
        <p>
          For that identified integration, the applicable order, service documentation, and this schedule form the
          written agreement between the parties. RateLoop will provide the documented service capabilities and
          limitations, relevant technical access and interfaces, reasonably necessary integration information and
          assistance, and information about the expertise supporting those services. The customer remains responsible
          for specifying what it needs to meet its obligations as provider. Both parties must cooperate in good faith
          within their respective technical control; this schedule is not an attestation and does not establish
          compliance.
        </p>
        ...
        <p>
          ...
          commit is accepted, a funder cannot cancel the round. Accepted work must reach the disclosed paid or
        ...
        <p>
          ...
          subscription payment details and provides invoices and receipts. Except where law requires otherwise or
          RateLoop agrees in writing, paid subscription periods are non-refundable.
        </p>
        ...
        <p>
          ...
          the final go, revise, or stop decision. RateLoop results are not financial, legal, medical, or investment
          advice and must not be used as an automatic approval for regulated or safety-critical decisions.
        </p>
        <p>
