        ...
        blocks: [
          {
            ...
            items: [
              ...
              "Settle: the public verdict closes once the configured reveal and participation conditions are met, then non-tied rounds finalize rewards after the relevant RBTS, public-rating, and payout roots are ready.",
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "Local signer tooling lets a Codex-like local agent use an encrypted keystore, sign EIP-3009 USDC authorization when required, execute returned calls, and confirm hashes; USDC Feedback Bonuses on native asks are funded by the same one-shot submit call, while wallet-call Feedback Bonus plans are executed and confirmed as a separate approve/create-pool follow-up.",
              ...
              "Wallet settings cover ETH for gas, while the agent setup screen can help fund LREP or USDC for wallet-call bounties.",
            ],
          },
        ],
      },
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "Correlation Epoch Snapshots cap RBTS stake-settlement weights, public-rating evidence, LREP or USDC bounty payouts, and launch LREP payouts for dense wallet or operator clusters across multiple rounds, with roots proposed by globally bonded 1,000 LREP frontend operators and recomputed from pinned source-event input snapshots.",
            ],
          },
        ],
      },
    ...
    subsections: [
      ...
      },
      {
        ...
        blocks: [
          ...
          {
            type: "paragraph",
            text: `RBTS settlement stores each revealed report's scoreBps, waits for the finalized RBTS settlement snapshot, computes a leave-one-out benchmark for each staked report from the effective-weighted scores of the other score-eligible revealed reports, and compares the report with that benchmark. The RBTS settlement snapshot caps detected clusters so a dense bloc shares no more effective RBTS settlement weight than the strongest member in that cluster before score-spread rewards and forfeits are computed. Positive spreads recover full stake plus a pro-rata share of the 96% voter share of forfeited negative-spread stake; negative spreads forfeit, with no revealed-loser rebate, only after ${protocolDocFacts.scoreSpreadForfeitMinRevealsLabel} effective score-eligible participant units are present. Per-report score-spread forfeiture is capped at ${protocolDocFacts.maxScoreSpreadForfeitPercentLabel} of stake. If the RBTS snapshot times out, revealed stake is returned without score-spread rewards or forfeits. The benefit is that stake rewards follow relative predictive quality rather than raw popularity, so raters have a reason to report independently instead of copying visible momentum; leave-one-out also prevents a large staker from pulling its own payout benchmark toward its score. This is an independent-rater Bayes-Nash incentive claim, not a proof that truthful reporting is dominant-strategy or uniquely payoff-maximizing against undetected coordination. LREP or USDC bounty claims and launch LREP claims add a second step: a finalized correlation payout snapshot proposed by a registered frontend operator supplies effective claim weights before funds move.`,
          },
          {
            type: "sub_heading",
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "Qualified bounty rounds pay eligible revealed raters and reserve 3% for eligible frontend operators after correlation-capped payout weights finalize; LREP stake rewards similarly wait for the RBTS settlement root.",
              ...
              "Bounty claims default open to everyone, while creators may select Proof-of-Human bounty eligibility when they want credential-gated claims; finalized claim weights, reputation, and calibration can still shape policy and routing.",
          ...
          {
            type: "formula",
            latex: String.raw`w_i = \mathrm{clusterBudget}_{c,s}\!\left(w_i^{\mathrm{base}}\right) \cdot \frac{\mathrm{ind}_i}{10\,000} \qquad w_i^{\mathrm{base}} = 10\,000 + \mathrm{anchor}_{s} \cdot \mathrm{ind}_i \cdot \frac{\mathrm{rawSurprise}_i - 10\,000}{10^8}`,
          },
          {
            type: "paragraph",
            text: "A_R is the round allocation, w_i is the finalized claim weight, w_i^base is the surprise-weighted base weight after the side anchor check, clusterBudget caps aggregate surprise bonus for detected same-side clusters, and ind_i is the independence multiplier in basis points from the correlation scorer. The benefit is a bounty rule that buys scarce, informative judgment instead of rewarding only participation or raw majority alignment. Surprise weighting is a bounty-payout rule, not a standalone truthfulness proof; it is strongest when paired with anchor checks, cluster budgets, the independence multiplier, and public auditability.",
          },
        ],
      },
      {
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "MCP-style tools include `rateloop_list_categories`, `rateloop_list_result_templates`, `rateloop_quote_question`, `rateloop_ask_humans`, `rateloop_confirm_ask_transactions`, `rateloop_confirm_feedback_bonus_transactions`, `rateloop_get_question_status`, and `rateloop_get_result`.",
              "Direct HTTP routes cover templates, quotes, ask creation, ask transaction confirmation, follow-up Feedback Bonus confirmation at `/api/agent/asks/{operationKey}/confirm-feedback-bonus`, status/result reads, and browser-signing intent create/read/prepare/complete endpoints under `/api/agent/signing-intents`.",
              ...
              "The local signer CLI loads an encrypted keystore, signs EIP-3009 USDC authorization when needed, sends returned transaction plan calls through viem, waits for receipts, confirms the ask, and confirms any wallet-call Feedback Bonus follow-up plan.",
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "Confirm submitted transaction hashes through `rateloop_confirm_ask_transactions`, then execute and confirm any returned Feedback Bonus follow-up plan through `rateloop_confirm_feedback_bonus_transactions` or `/api/agent/asks/{operationKey}/confirm-feedback-bonus`.",
    ...
    subsections: [
      ...
      },
      {
        ...
        blocks: [
          {
            type: "paragraph",
            text: "The ClusterPayoutOracle is a governance-managed target for challengeable correlation epoch and round payout roots. Registered frontend operators with a 1,000 LREP bond propose deterministic artifact roots from their registered wallet or a delegated snapshot keeper; artifact v3 commits each epoch parameter hash to both scorer parameters and pinned source-event input snapshot references. Independent operators can recompute them and challenge bad roots with the configured USDC ERC20 challenge bond, defaulting to 5 USDC (5_000_000 atomic units). Governance configures oracle challenge and finalization-veto terms, arbitrates challenged roots with public reason hashes, and can slash the proposing frontend through the FrontendRegistry when the on-chain-data computation is wrong. An escrow or launch consumer may act after a payout root is finalized and eligible, but governance can still reject a consumed root until the finalization veto window elapses; after that window, the consumed root is pinned for that consumer path. Longer review windows give challengers and voters more public audit time at the cost of slower healthy finality, while shorter windows improve claim speed but narrow the review period.",
          },
          {
            type: "paragraph",
        ...
        blocks: [
          {
            ...
            items: [
              ...
              "The current single-task RBTS mechanism is Bayes-Nash truthful under independent-rater assumptions, but not collusion-proof; undetected coordinated blocs remain a residual signal risk despite RBTS/public-rating correlation roots and surprise-weighting mitigations.",
