...
export const EXECUTIVE_SUMMARY: ContentBlock[] = [
  ...
  {
    type: "paragraph",
    text: "The protocol turns rating into an explicit peer-prediction round. Every ask is question-first, requires inspectable context through a public URL, image, YouTube video, or RateLoop-hosted gated context, and carries a non-refundable bounty funded in LREP or USDC. Everyone can answer public asks; gated asks require confidentiality access checks before context is served. The bounty payout can remain open or be scoped to verified humans. Raters submit a private thumbs-up/down signal plus a 0-100% prediction of how many raters will vote up. Zero-LREP advisory votes can participate only in rounds that already have a staked vote, do not count toward settlement quorum, and can qualify for launch credits in eligible settled rounds; staked votes add normal settlement upside and downside. Optional written feedback is public on-chain when submitted, and eligible revealed raters claim bounty payouts after challengeable frontend-backed correlation payout snapshots finalize.",
  },
  {
    type: "paragraph",
    text: `Signal integrity comes from combining calibration, optional LREP-backed stake, identity signals, blind rounds, correlation-adjusted RBTS/public-rating weights, and correlation-capped payouts. Votes and population predictions stay hidden through tlock until the blind epoch ends, later raters earn only ${protocolDocFacts.openPhaseWeightLabel} reward weight instead of ${protocolDocFacts.blindPhaseWeightLabel}, and settlement waits for at least three reveals plus the configured reveal conditions so visible copycat herding and selective reveal are harder. Correlation Epoch Snapshots proposed by registered frontend operators set effective RBTS settlement weights, public-rating evidence weights, and USDC/launch LREP payout weights from pinned source-event input snapshots; truthfulness remains an independent-rater Bayes-Nash guarantee, not a collusion-proof claim.`,
  },
  {
    type: "paragraph",
