...
const FORBIDDEN_PUBLIC_SECRETS = [
  ...
  // The integrity family had only its first key name listed, so the signing
  // private key, the JSON keyring, the pseudonym key and the vault key could all
  // have been shipped to the browser without the build failing. Every name here
  // is read from `process.env` by production code in `integrityEpochs`,
  // `integrityEpochProducer`, `audienceAssignments` or `crowdForecastPersistence`.
  "NEXT_PUBLIC_TOKENLESS_INTEGRITY_SIGNING_PRIVATE_KEY",
  "NEXT_PUBLIC_TOKENLESS_INTEGRITY_REVIEWER_LOOKUP_KEYS_JSON",
  "NEXT_PUBLIC_TOKENLESS_INTEGRITY_REVIEWER_LOOKUP_KEY_VERSION",
  "NEXT_PUBLIC_TOKENLESS_INTEGRITY_PSEUDONYM_KEY",
  "NEXT_PUBLIC_TOKENLESS_INTEGRITY_PSEUDONYM_KEY_VERSION",
  "NEXT_PUBLIC_TOKENLESS_INTEGRITY_VAULT_KEY",
  "NEXT_PUBLIC_TOKENLESS_INTEGRITY_VAULT_KEY_VERSION",
  ...
  "NEXT_PUBLIC_TOKENLESS_COMPLIANCE_OPERATOR_SECRET",
...
function validateTokenlessTestDeployment(env, { activeRegistry, deploymentSchema, deploymentStatus }) {
  ...
  const integrityReviewerLookupKey = decode32(value(env, "TOKENLESS_INTEGRITY_REVIEWER_LOOKUP_KEY"), "base64url");
  if (!integrityReviewerLookupKey) {
    errors.push("TOKENLESS_INTEGRITY_REVIEWER_LOOKUP_KEY must encode exactly 32 bytes.");
  }
  if (!/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(value(env, "TOKENLESS_INTEGRITY_REVIEWER_LOOKUP_KEY_VERSION"))) {
    errors.push(
