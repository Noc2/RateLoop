...
const RANK_BY_RATING_TEMPLATE_IDS = new Set([
...
export function readHeadToHeadTemplateInputs(
  templateInputs: unknown,
): HeadToHeadVoteUi | null {
...
function buildInferredHeadToHeadAbTitle(
  optionALabel: string,
  optionBLabel: string,
) {
...
function inferHeadToHeadAbQuestionFromCandidate(
  source: string,
): InferredHeadToHeadAbQuestion | null {
  for (const pattern of OPTION_PAIR_PATTERNS) {
    ...
    if (!optionALabel || !optionBLabel || optionALabel === optionBLabel)
      continue;
    return {
      optionALabel,
      optionBLabel,
...
export function inferHeadToHeadAbQuestionFromText(
  text: string,
): InferredHeadToHeadAbQuestion | null {
...
export function inferHeadToHeadAbQuestion(
  question: unknown,
  inheritedTemplateId?: unknown,
): InferredHeadToHeadAbQuestion | null {
  if (!isRecord(question)) return null;
  const templateId =
    readTrimmedString(question.templateId) || readTrimmedString(inheritedTemplateId);
  if (
...
export function normalizeInferredHeadToHeadAbRequestBody(
  ...
  if (isRecord(question)) {
    const normalized = normalizeInferredHeadToHeadAbQuestion(
      question,
...
export function readHeadToHeadVoteUiFromQuestionMetadata(
  metadata: unknown,
): HeadToHeadVoteUi | null {
...
export function inferHeadToHeadVoteUiFromText(
  text: string,
): HeadToHeadVoteUi | null {
...
export function resolveVoteUiConfig(params: {
  ...
  const fromMetadata = readHeadToHeadVoteUiFromQuestionMetadata(
