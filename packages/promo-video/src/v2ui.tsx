...
/**
 * Primitives mirroring real site components for the v2 promo scenes.
...
export const HeroThumbIcon = ({ up, size = 38, color }: { up: boolean; size?: number; color: string }) => (
  <svg
    viewBox="0 0 24 24"
    width={size}
    height={size}
...
export const VoteButtonV2 = ({ up, pressed = false, scale = 1 }: { up: boolean; pressed?: boolean; scale?: number }) => {
...
export const GradientActionButtonV2 = ({
  label,
  frame,
  startFrame = 0,
  pressedAt,
...
export const OutlineButtonV2 = ({ label, startFrame = 0 }: { label: string; startFrame?: number }) => {
  const entrance = useFadeInUp(startFrame, 12);
  return (
    <span
      style={{
