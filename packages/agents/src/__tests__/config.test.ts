...
describe("agents runtime config", () => {
  it("loads the tokenless public wallet address from env", () => {
    process.env.CURYO_AGENT_WALLET_ADDRESS = "0x00000000000000000000000000000000000000aa";
  ...
  it("rejects invalid tokenless public wallet addresses", () => {
    process.env.CURYO_AGENT_WALLET_ADDRESS = "not-an-address";

    expect(() => loadAgentsRuntimeConfig()).toThrow("CURYO_AGENT_WALLET_ADDRESS must be a valid EVM address");
  });
});
