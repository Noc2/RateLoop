...
Public raw MCP asks can attach signed callbacks by passing `webhookUrl` and `webhookSecret`, signing the returned `webhook_signature_required` message with the paying wallet, then retrying with `webhookChallengeId` and `webhookSignature`. Supported events are `question.submitting`, `question.submitted`, `question.open`, `question.settling`, `question.settled`, `question.failed`, `feedback.unlocked`, and `bounty.low_response`; omit `webhookEvents` to subscribe to all supported events. `question.failed` is emitted for prepare-time ask submission failures before the question reaches the chain; post-submission confirmation or settlement blockers should be handled through polling status and result fields. Callback deliveries include `x-rateloop-callback-id`, `x-rateloop-callback-timestamp`, and `x-rateloop-callback-signature: v1=<hex>`, where the signature is HMAC-SHA256 keyed by `webhookSecret` over `v1.{callback-id}.{callback-timestamp}.{rawBody}` using the exact received body bytes. Status responses report retry state in `callbackDeliveries[].status`, which is separate from ask/result polling status. Managed agents can also call `rateloop_get_agent_balance`; balance caps and audit exports require a saved policy and bearer token.
...
material. For gated private context, require a RateLoop-hosted `detailsUrl` (and matching `detailsHash`); optional hosted `imageUrls` are allowed. Keep the public title
...
When a bundle needs repeated samples, submit fresh bundled asks with the intended creation-time budget. Each bundled ask
uses one creation-anchored rewardable round set: every bundled question must settle once before that set can pay.
