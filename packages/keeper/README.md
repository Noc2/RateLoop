...
Set `KEEPER_DATABASE_URL` to enable optional keeper persistence. The keeper creates a small `keeper_correlation_artifacts` table for automatic correlation artifact cache rows and uses Postgres advisory locks to prevent overlapping Railway replicas or deployments from doing the same work at the same time. In production, `KEEPER_MAIN_LOOP_LOCK_REQUIRED` defaults to `true`; if the database or advisory lock is unavailable, the keeper skips the main loop tick instead of running duplicate settlement/reveal work. Local and explicitly opted-out deployments keep the previous stateless fallback behavior, and on-chain status checks still prevent duplicate proposals.
