...
| `FEEDBACK_REGISTRY_ADDRESS`                       | Auto-derived for supported chains                                   | Local `31337` override only; live chains require shared deployment artifacts                                       |
...
| `KEEPER_PROACTIVE_ROUND_OPENING_ENABLED`          | `false`                                                             | Opt into bounded background `openRound` calls for recent public content so first voters skip the separate round-open transaction |
| `KEEPER_PROACTIVE_ROUND_OPENING_MAX_PER_TICK`     | `2`                                                                 | Max proactive `openRound` calls per keeper tick                                                                    |
| `KEEPER_PROACTIVE_ROUND_OPENING_RECENT_SECONDS`   | `21600`                                                             | Only ask Ponder for unopened content active within this many seconds                                               |
| `KEEPER_REWARD_POOL_QUALIFICATIONS_ENABLED`       | `true`                                                              | Enable Ponder-driven background `qualifyRound` and question-bundle terminal sync calls                             |
| `KEEPER_REWARD_POOL_QUALIFICATIONS_PER_TICK`      | `25`                                                                | Max single-question reward pool rounds to qualify per keeper tick                                                  |
| `KEEPER_BUNDLE_TERMINAL_SYNCS_PER_TICK`           | `10`                                                                | Max question bundle terminal-sync transactions per keeper tick                                                     |
| `KEEPER_BUNDLE_TERMINAL_SYNC_MAX_ROUNDS`          | `25`                                                                | Max per-bundle internal rounds passed to `syncQuestionBundleTerminals(bundleId, maxRounds)`                        |
...
| `KEEPER_MAIN_LOOP_LOCK_REQUIRED`                  | `true` in production, otherwise `false`                             | Require the Postgres advisory lock before running settlement/reveal work; production replicas should keep this on  |
| `KEEPER_CORRELATION_SNAPSHOT_LOCK_REQUIRED`       | `true` in production when correlation snapshots are enabled, otherwise `false` | Require the Postgres advisory lock before publishing correlation snapshots; skip publication instead of running without the lock when unavailable or busy |
| `METRICS_ENABLED`                                 | `true`                                                              | Enable Prometheus metrics server                                                                                   |
| `METRICS_BIND_ADDRESS`                            | `0.0.0.0` when `PORT` is set, otherwise `127.0.0.1`                 | Metrics server bind address; set `METRICS_AUTH_TOKEN` before exposing it publicly                                  |
| `METRICS_PORT`                                    | `PORT` when set, otherwise `9090`                                   | Metrics server port                                                                                                |
...
| `KEEPER_CORRELATION_SNAPSHOT_MAX_ROUNDS_PER_TICK` | `20`                                                                | Max settled LREP or USDC bounty rounds to score per keeper tick                                                     |
...
When `KEEPER_FRONTEND_FEE_ENABLED=true`, the same worker prioritizes a bounded cursor through recent settled rounds for the configured frontend/operator, then backfills older settled rounds so historical `RoundRewardDistributor.claimFrontendFee(...)` claims do not age out of automation. It can also drive the two-step registry withdrawal: it completes a matured `FrontendRegistry.completeFeeWithdrawal()` first when no snapshot dispute is active, then moves newly accrued fees into the next pending bucket with `requestFeeWithdrawal()`. Requested amounts stay slashable for the registry's 1-hour `FEE_WITHDRAWAL_DELAY`, and completion remains paused while the frontend has an active challenged payout snapshot; voluntary frontend stake exits still wait out the separate 14-day unbonding period.

When `KEEPER_CORRELATION_SNAPSHOTS_ENABLED=true`, the worker checks that the keeper wallet resolves through `FrontendRegistry.authorizedSnapshotFrontend(...)` to an eligible frontend operator only when a missing/rejected proposal slot actually needs a proposal. The keeper wallet can be the registered frontend wallet itself, or a separate operational wallet assigned by that frontend operator. The worker proposes missing correlation epoch and round payout roots from the keeper wallet and finalizes already-proposed roots after the challenge window. In `auto` mode it first preflights on-chain status so already-proposed/finalized snapshots do not rebuild artifacts, then asks Ponder for settled LREP or USDC bounty rounds, pending RBTS settlement rounds, and pending earned launch credits only when proposal data is needed, builds deterministic payout weights with `@rateloop/node-utils/correlationScoring` (question-reward weights use surprise, verified-anchor, cluster-budget, and independence stages; RBTS settlement weights cap detected clusters by effective reveal weight; launch-credit weights stay flat and require zero identity keys), stores the public artifact, and publishes the roots. Ponder returns the correlation inputs snapshotted at the public verdict close, RBTS settlement-pending event, or pending launch-credit event; missing snapshots fail closed instead of letting a later human credential, vote count, or ban change alter an old payout root. The public artifact version is `rateloop-correlation-artifact-v3`, and each epoch `parameterHash` commits to the scoring params plus those input snapshot references. In `file` mode it reads the same artifact shape from `KEEPER_CORRELATION_SNAPSHOT_ARTIFACT_PATH`, and `yarn workspace @rateloop/keeper verify:correlation-artifact <artifact.json>` enforces the same launch-credit contract shape and pinned-input hashes before publication.

Set `KEEPER_DATABASE_URL` to enable optional keeper persistence. The keeper creates a small `keeper_correlation_artifacts` table for automatic correlation artifact cache rows and uses Postgres advisory locks to prevent overlapping Railway replicas or deployments from doing the same work at the same time. In production, `KEEPER_MAIN_LOOP_LOCK_REQUIRED` defaults to `true`; if the database or advisory lock is unavailable, the keeper skips the main loop tick instead of running duplicate settlement/reveal work. When correlation snapshots are enabled, `KEEPER_CORRELATION_SNAPSHOT_LOCK_REQUIRED` defaults to `true` in production as well; if the correlation snapshot lock is unavailable or held by another replica, publication is skipped for that tick instead of running without the lock. Local and explicitly opted-out deployments keep the previous stateless fallback behavior, and on-chain status checks still prevent duplicate proposals.

### Production Ponder dependency

The keeper treats Ponder as a hard production dependency for work discovery, deployment verification, indexed vote ciphertext fetches, and auto correlation snapshot builds. In production (`NODE_ENV=production`), failures on Ponder `/keeper/work`, `/deployment`, or keeper-work auth fail the entire tick instead of falling back to chain-only enumeration. That fail-closed behavior prevents the keeper from acting against a stale or misconfigured indexer, but it also means a Ponder outage or misconfigured `PONDER_KEEPER_WORK_TOKEN` stops settlement, reveals, cleanup, reward-pool qualification, bundle terminal sync, forfeits, and correlation snapshot publication until Ponder recovers.

After repointing Ponder to a new contract deployment, restart every keeper process. In production the keeper checks Ponder `/deployment` before each keeper-work and ciphertext-fetch interaction and fails closed on any chain, registry, or deployment-key mismatch, so cutovers should restart all replicas once the new Ponder deployment is serving the replacement metadata.

For `yarn dev:stack` on local `31337`, set `KEEPER_CORRELATION_SNAPSHOTS_ENABLED=true`, `KEEPER_CORRELATION_SNAPSHOTS_MODE=auto`, and `KEEPER_CORRELATION_ARTIFACT_STORAGE=data-uri` in `packages/keeper/.env.local`; keep `PONDER_BASE_URL=http://localhost:42069`. Before starting the stack, run `yarn chain`, `yarn deploy`, register the frontend operator, and either run the keeper from that registered wallet or set a separate unregistered keeper address in frontend settings. For Base mainnet production, use the owner-directed fresh deployment artifacts for the current replacement stack, run `yarn base-mainnet:check` against the same deployment key, then deploy or reconfigure Ponder, Next.js, and the keeper around that deployment. Register the production frontend operator; set `KEEPER_CORRELATION_ARTIFACT_STORAGE=file`, `KEEPER_CORRELATION_SNAPSHOT_STORAGE_DIR` to a persistent directory, and `KEEPER_CORRELATION_SNAPSHOT_PUBLIC_BASE_URL` to the keeper's public `/correlation-artifacts` route or another public HTTPS artifact host. Set Ponder's `PAYOUT_ARTIFACT_HTTPS_ALLOWLIST` to the same HTTPS prefix so claim-proof routes can fetch the keeper-published artifacts. The operator must keep the 1,000 LREP frontend bond active. Anyone can run the same artifact through the same scorer and challenge mismatched roots on-chain; governance can arbitrate challenged roots and slash the frontend if the computation was wrong.

When the keeper's metrics server is enabled, it serves hash-named artifact files from `KEEPER_CORRELATION_SNAPSHOT_STORAGE_DIR` at `/correlation-artifacts/<artifactHash>.json`. Metrics and readiness requests still use `METRICS_AUTH_TOKEN`; `/live` is an unauthenticated local liveness probe for container healthchecks. Artifact files are public because their URI and hash are published on-chain. On Railway, attach a Postgres database and set the keeper service's `KEEPER_DATABASE_URL` to the Postgres service's `DATABASE_URL` reference for cache/locking. Also mount a volume on the keeper service, point `KEEPER_CORRELATION_SNAPSHOT_STORAGE_DIR` at that mount path, expose the keeper service publicly, and use `https://<keeper-domain>/correlation-artifacts` as both `KEEPER_CORRELATION_SNAPSHOT_PUBLIC_BASE_URL` and Ponder's `PAYOUT_ARTIFACT_HTTPS_ALLOWLIST`. The Postgres cache stores canonical artifact JSON so a cache hit can re-materialize the file on the mounted volume if needed; the volume remains the public artifact store.

### Work discovery liveness

With `KEEPER_WORK_DISCOVERY_PONDER_ENABLED=true` (default), most ticks ask Ponder for up to `KEEPER_WORK_DISCOVERY_MAX_CANDIDATES` urgent items **per category** (open rounds, cleanup hints, dormancy, reward-pool qualifications, reward-pool residue sweeps, bundle terminal syncs, Feedback Bonus forfeits — up to ~7× the limit per response). Content outside those capped sets would otherwise wait until the next full chain reconciliation (`KEEPER_WORK_DISCOVERY_RECONCILE_EVERY_TICKS`, default 120 ticks). Each Ponder tick also advances a bounded chain scan: `KEEPER_WORK_DISCOVERY_CHAIN_SCAN_PER_TICK` content IDs from a rotating cursor (default `max(10, ceil(500/120)) = 10`), so every registered content ID is visited periodically even when Ponder prioritization is saturated.

`KEEPER_PROACTIVE_ROUND_OPENING_ENABLED=true` adds a separate, capped `roundOpenRequests` bucket to `/keeper/work`. Ponder returns recent active, public, unbundled content with no current open round and no prior empty round record; the keeper re-checks on-chain `currentRoundId` before broadcasting `openRound`. The keeper wallet must be eligible to open the target round: public content works with an unbanned address-derived rater identity, while gated content and submitter-owned content are skipped by contract validation. Keep the per-tick cap low in production until gas and latency metrics look healthy.

`KEEPER_REWARD_POOL_QUALIFICATIONS_ENABLED=true` lets Ponder surface already-settled bounty rounds that are not yet qualified, expired question reward pools with unallocated residue to sweep through `refundExpiredRewardPool`, plus bundle rewards with terminal member rounds ready to sync. The keeper re-reads the current `ContentRegistry.questionRewardPoolEscrow()` address, processes at most `KEEPER_REWARD_POOL_QUALIFICATIONS_PER_TICK` single-pool `qualifyRound` calls and expired reward-pool residue sweeps, processes at most `KEEPER_BUNDLE_TERMINAL_SYNCS_PER_TICK` bundle sync calls, and treats stale indexed candidates as benign estimation skips. This reduces first-claim/refund gas spikes and clears expired bounty residue without increasing oracle challenge windows or other user-facing waits.
