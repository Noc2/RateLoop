...
beforeAll(async () => {
  ...
  process.env.TOKENLESS_DEPLOYMENT_KEY = `tokenless-v1:31337:${PANEL}:${ISSUER}:${ZERO}`;
...
describe("tokenless keeper config", () => {
  it("builds the versioned isolated deployment identity", () => {
    ...
    ).toBe(`tokenless-v1:84532:${PANEL}:${ISSUER}:${ZERO}`);
  });
  ...
  it("rejects mainnet and mixed deployment keys", () => {
    ...
    expect(() =>
      loadConfig({
        ...productionEnv(),
        TOKENLESS_DEPLOYMENT_KEY: `tokenless-v1:84532:${ISSUER}:${PANEL}:${ZERO}`,
      })
