...
export async function listActiveFollowedAddresses(follower: `0x${string}`) {
  const rows = await db
    ...
    .from(raterFollow)
    .where(
      and(eq(raterFollow.follower, follower), eq(raterFollow.active, true)),
    );
...
export async function getFollowStatsMap(addresses: readonly `0x${string}`[]) {
  const uniqueAddresses = [
    ...new Set(
      addresses.map((address) => address.toLowerCase() as `0x${string}`),
    ),
  ];
  ...
  const [followers, following] = await Promise.all([
    db
      ...
      .where(
        and(
          eq(raterFollow.active, true),
          inArray(raterFollow.target, uniqueAddresses),
        ),
      )
    ...
    db
      ...
      .where(
        and(
          eq(raterFollow.active, true),
          inArray(raterFollow.follower, uniqueAddresses),
        ),
      )
  ...
  for (const row of followers) {
    const existing = stats.get(row.address as `0x${string}`) ?? {
      followerCount: 0,
      followingCount: 0,
    };
