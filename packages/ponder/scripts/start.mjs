...
import { buildTokenlessDeploymentKey, resolveTokenlessDeployment } from "../src/protocol-deployment.ts";
...
const env = { ...process.env, RATELOOP_PONDER_PROTOCOL_DEPLOYMENT_KEY: deploymentKey };
const schema = resolvePonderDatabaseSchema(env);
const child = spawn("ponder", ["start", "--schema", schema, ...process.argv.slice(2)], {
  env,
  stdio: "inherit",
});
