...
| Next.js browser writes + server receipt reads | `NEXT_PUBLIC_USE_BASE_PRECONF_RPC`       | Set to `true` on Vercel/Railway only when the chosen preconfirmation provider is acceptable for receipt reads |
| Next.js browser writes + server receipt reads | `NEXT_PUBLIC_BASE_PRECONF_RPC_URL_8453`  | Dedicated Base mainnet Flashblocks/preconfirmation RPC override for deployed transaction UX                  |
| Next.js browser writes + server receipt reads | `NEXT_PUBLIC_BASE_PRECONF_RPC_URL_84532` | Dedicated Base Sepolia Flashblocks/preconfirmation RPC override for staging transaction UX                   |
