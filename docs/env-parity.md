...
| Next.js browser writes/receipt reads | `NEXT_PUBLIC_USE_BASE_PRECONF_RPC`       | Set to `true` on Vercel/Railway to use Base Flashblocks/preconfirmation RPC metadata for browser wallet progress |
| Next.js browser writes/receipt reads | `NEXT_PUBLIC_BASE_PRECONF_RPC_URL_8453`  | Dedicated Base mainnet Flashblocks/preconfirmation RPC override for deployed transaction UX                      |
...
| Next.js server receipt reads         | `RATELOOP_SERVER_USE_BASE_PRECONF_RPC`   | Optional explicit opt-in for server confirmation to use Base preconfirmation metadata; default is ordinary RPCs   |
| Next.js server receipt reads         | `RATELOOP_SERVER_BASE_PRECONF_RPC_URL_8453`  | Optional dedicated Base mainnet server preconfirmation RPC override when the server opt-in is enabled         |
| Next.js server receipt reads         | `RATELOOP_SERVER_BASE_PRECONF_RPC_URL_84532` | Optional dedicated Base Sepolia server preconfirmation RPC override when the server opt-in is enabled         |
...
The deployed Next.js app can use Base Flashblocks/preconfirmation RPCs to make wallet progress appear quickly after a user confirms. `NEXT_PUBLIC_USE_BASE_PRECONF_RPC=true` is a public browser env toggle; Next.js server confirmation code ignores it and stays on ordinary RPC metadata by default. Only set `RATELOOP_SERVER_USE_BASE_PRECONF_RPC=true` after confirming the provider's receipt APIs are acceptable for server confirmation calls. When the dedicated preconfirmation RPC env var is empty, the app prefers configured ordinary browser RPCs, Alchemy, and Base's standard public RPCs before using Base's public preconfirmation endpoint as the last fallback. Ponder and Keeper should use ordinary sealed-block RPCs for canonical indexing and automation. Thirdweb client and verifier env vars configure wallet, top-up, and sponsorship behavior; do not treat them as the canonical indexer RPC or the Flashblocks/preconfirmation provider by default.
