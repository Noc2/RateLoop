...
| Deployment | Disposable Base Sepolia v2 bundle, generated exports, isolated Vercel app, Ponder, and keeper share one complete deployment key; provider-neutral Better Auth plus opaque RateLoop principals are implemented, while wallets remain optional purpose-bound adapters; app remains sandbox-only | Apply every migration through `0044`, provision Better Auth and EU email delivery, verify OTP/passkey and optional social methods, enable and verify thirdweb only if managed wallet creation is required, complete paid live E2E and non-sandbox provider/secret configuration, and redeploy atomically after any later fund-core change |
...
4. **Secret/provider provisioning.** The non-sandbox bundle still needs the issuer signer, distinct relayer + prepaid-funder keys, pipeline/webhook/keeper-work tokens, two Postgres URLs, and the eligibility-provider bundle. Browser authentication requires server-only `BETTER_AUTH_SECRET`, the exact hosted `BETTER_AUTH_PASSKEY_RP_ID`, and a configured Resend sender for email OTP; Google and Apple remain optional paired credentials. thirdweb is not account authentication: when optional wallet creation is enabled, configure only `NEXT_PUBLIC_THIRDWEB_CLIENT_ID` plus the audience, key ID, and server-only Ed25519 wallet JWT key. Decide what fills the eligibility-provider slots on testnet and apply every journaled drizzle migration through `0044_provider_neutral_identity` before `TOKENLESS_SANDBOX_MODE=false`.
