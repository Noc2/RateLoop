...
4. **The mechanism survives without LREP.** Reputation becomes non-transferable points; frontend-operator and confidentiality bonds become USDC deposits; governance becomes a published upgrade policy (the Base/Farcaster posture, which is the accepted 2026 norm). Vote stakes are **removed entirely rather than converted** — German gambling law makes stake-forfeiture-to-winners untenable and stake-free crowd judgment (Community Notes, Code4rena, Sherlock, Metaculus) is the established norm; see the [implementation plan](tokenless-immutable-implementation-plan-2026-07.md) §A.2/§A.4 for the analysis and the replacement (identity tiers, probationary reputation, zero-floor scoring).
