...
**Follow-up:** the concrete repo conversion plan (keyless immutable design, the gambling-law analysis that led to removing vote bonds entirely, latency/commit-reveal decision) lives in [tokenless-immutable-implementation-plan-2026-07.md](tokenless-immutable-implementation-plan-2026-07.md); the consolidated legal/revenue assessment is [legal-revenue-assessment-tokenless-design-2026-07.md](legal-revenue-assessment-tokenless-design-2026-07.md), and the adversarial review of the whole plan is [tokenless-plan-review-multi-agent-2026-07.md](tokenless-plan-review-multi-agent-2026-07.md). Where this document conflicts with those, they win — several passages below reflect earlier decision rounds.
...
4. **The mechanism survives without LREP.** Reputation becomes non-transferable points; the frontend-operator bond is deleted (attribution-only; a bonded program can return later) and rater-side confidentiality bonds are dropped (identity sanctions carry the deterrence); governance disappears rather than decentralizing — fully keyless immutable contracts, bug response = disclose → exit → redeploy (the Liquity model). Vote stakes are **removed entirely rather than converted** — German gambling law makes stake-forfeiture-to-winners untenable and stake-free crowd judgment (Community Notes, Code4rena, Sherlock, Metaculus) is the established norm; see the [implementation plan](tokenless-immutable-implementation-plan-2026-07.md) §A.2/§A.4 for the analysis and the replacement (identity tiers, probation for unverified wallets, gold questions gating eligibility attestations — never in-round settlement — and soft score transforms; per the plan reviews, a hard zero-floor punishes honest minorities).
...
- A **5–10% interface fee on bounty flows** is comfortably inside all norms and undercuts web2 judgment marketplaces by 2–4×. Percentage fees fit marketplace-sized bounties; **flat-cent fees (x402-style) fit agent micro-asks** — support both shapes.
...
- German VAT on the *fee itself* is ordinary consideration for a service — 19% where applicable, reverse charge cross-border B2B. **Superseding note:** the later legal assessment (§2.1) found a much bigger VAT issue — Fenix/Xyrality deemed-supplier risk on the *full bounty flow* — which is a pre-launch structuring gate, not "unexciting."
...
|---|---|---|
| Vote stake (1–10 LREP, forfeited on non-reveal) | **No stake at all** — free entry with identity-tier gating, probation for unverified wallets, and soft score-transform payouts that pay uninformative votes ≈ nothing (stake-forfeiture-to-winners is a German gambling-law trap; zero rater payment removes the Entgelt element entirely) | Community Notes, Code4rena, Sherlock, Metaculus — all stake-free; Gitcoin cost-of-forgery gating |
...
| Confidentiality bond + identity sanction | Identity sanction (nullifier ban) only — rater-side confidentiality bonds were dropped in review (they contradicted "raters never deposit funds"); the confidential lane is restricted to identified raters instead | Plan Part B; review L5/L9 |
| Governance (Governor + timelock + LREP votes) | Deleted entirely: keyless, non-upgradeable contracts — no multisig, no timelock, no upgrade path; bug response = disclose → exit → redeploy | Liquity (governance-free); plan §A.1 |
...
| Challenge bonds / challenger rewards (ClusterPayoutOracle) | Delete the on-chain challenge game (theater with a single operator) — settle directly, publish inputs + open-source recompute script as a transparency log, apply correlation analytics off-chain to future-round eligibility | Gitcoin runs COCM off-chain with public review; Community Notes publishes algorithm + data; L2Beat treats operator-only challenge sets as Stage-0 |
...
2. **Sybil defense shifts from money-at-stake to identity and scoring.** With free entry, the load-bearing controls are identity tiers whose forgery cost exceeds capped per-identity extraction (World ID; Gitcoin's cost-of-forgery math), probation for unverified wallets, minimum round sizes, and soft score-transform payouts that pay uninformative votes ≈ nothing — see the implementation plan §A.4/Phase 0.3.
3. **The Kleros argument** (a native token makes 51%-stake-buy attacks self-punishing and fork-recoverable) applies weakly here: RateLoop's rounds are small and per-question, eligibility gating bounds entry, and any dispute backstop can be outsourced. The Gauntlet counterpoint also applies: stablecoin-denominated economics don't collapse when a token dumps.
...
**Delete:** `LoopReputation.sol` (LREP), `RateLoopGovernor` + timelock, `LaunchDistributionPool`, LREP paths in escrows/registries, LREP staking in `RoundVotingEngine` (removed entirely — stake-free voting), the 1,000-LREP frontend bond (deleted — attribution-only), and `ClusterPayoutOracle` + correlation-snapshot machinery (decided in plan §A.5: single-operator challenge games are theater; a commitment-based transparency log replaces it).

**Keep (thin rail):** USDC bounty/feedback/confidentiality escrows with immutable fund paths and a capped caller-set interface-fee field; `X402QuestionSubmitter` (EIP-3009 agent lane — this *is* the x402 story); the backend-issued credential gate in `RaterRegistry` (World ID verified off-chain); commit-reveal voting (free entry, no rater funds, per-round pseudonymous vote keys); Base as the chain (tokenless L2, cheap, x402-native).

**Consequences:** governance runbooks, launch-pool logic, LREP tokenomics, and the correlation-oracle challenge economy disappear; keeper and ponder shrink to settlement + indexing of a much smaller contract set; the frontend loses all token UX (locks, governance, launch credits) and keeps wallet + World ID + bounty flows. Rough guess: half or more of the Solidity surface and most of the governance/ops burden goes away, while the regulatory posture *improves* (fewer admin powers, no token). No migration needed — there are no users; redeploy fresh.

**Positioning follows the architecture:** describe the product as "verified-human panels for agents and brands — minutes-fast, pay-per-question, provably sealed" (the rail is an implementation detail everywhere a buyer looks; round-2 review) — the Polymarket/Farcaster framing where the chain is an implementation detail, never the pitch. The word "token" should not appear anywhere.

## 6. Risks and open questions

1. **EMT/PSD2 "on behalf of clients"** — a gating counsel question (§2.2); the consolidated counsel list (eight open questions, VAT deemed-supplier the heaviest) is in the legal assessment Part 4. High uncertainty; no on-point precedent found.
2. **Immutability discipline is now load-bearing twice** — for the custody analysis and for user credibility (the April 2026 Drift exploit, a socially-engineered security-council multisig, made "who holds the keys" a live user question; the answer here is "nobody"). No upgrade path exists; the published policy is disclose → exit → redeploy.
3. **World ID association** remains RateLoop's largest residual HN/PR attack surface (WLD −97%, country bans, account black market where verified accounts sell for <$30). Mitigations: it's optional, it's one tier, we hold no biometric or document data (DAC7 identification data sits in a separate retention-designed vault — see the legal assessment), and face-auth rechecks are improving. Monitor; keep the credential layer pluggable.
