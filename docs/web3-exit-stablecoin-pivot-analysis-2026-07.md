...
|---|---|---|
| Vote stake (1–10 LREP, forfeited on non-reveal) | **No stake at all** — free entry with identity-tier gating, probationary reputation, and score-weighted payouts that zero out uninformative votes (stake-forfeiture-to-winners is a German gambling-law trap; zero rater payment removes the Entgelt element entirely) | Community Notes, Code4rena, Sherlock, Metaculus — all stake-free; Gitcoin cost-of-forgery gating |
...
2. **Sybil defense shifts from money-at-stake to identity and scoring.** With free entry, the load-bearing controls are identity tiers whose forgery cost exceeds capped per-identity extraction (World ID; Gitcoin's cost-of-forgery math), probationary reputation, minimum round sizes, and payout floors that zero uninformative votes — see the implementation plan §A.4.
...
1. **EMT/PSD2 "on behalf of clients"** — a gating counsel question (§2.2); the consolidated counsel list (six questions, VAT deemed-supplier the heaviest) is in the legal assessment Part 4. High uncertainty; no on-point precedent found.
