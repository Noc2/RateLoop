...
what knowledge a review needs in **Reviews** and establishes who can satisfy it in **People**. RateLoop may show
common examples, but suggestions are never selected, granted, or required without an explicit owner action.
...
**People** shows live coverage for each frozen requirement, separated into confirmed members and pending
invitations. It is where an owner invites reviewers, confirms expertise, sets qualification expiry, and recovers
from insufficient coverage. Pending, expired, revoked, removed, or unconfirmed people never count as coverage.
Network coverage is read-only and includes only active platform-verified global credentials.

An invitation may record the expertise the owner intends to confirm, but it is not a qualification. Redemption
creates private-group membership only. After redemption, the owner must explicitly attest the signed-in member's
workspace qualifications; a transferable invitation token, email binding, or reviewer self-declaration can
never grant expertise. An owner cannot grant a global network credential.
...
Private cohort provenance is a self-contained projection of canonical qualifications for assignment evidence,
not a second grant. Projection and membership refreshes must merge active expertise rather than erase it and
may never extend the canonical qualification or membership expiry.
