...
1. Provision new resources rather than reusing or relocating preview resources: EU Postgres and backups, a
   private EU Blob store, EU log sink, EU keeper, and EU Ponder. Keep server-only keys in the isolated Vercel and
   Railway projects; no RateLoop AWS account or KMS/IAM/OIDC integration is part of this bundle.
...
6. Inventory every server-only signing and wrapping key in Vercel and Railway. Record its role, public identity,
   version, creation time, rotation owner, and dependent envelopes or balances. Require distinct keys for artifact
   wrapping, evidence, credential issuance, relaying, funding, bonus funding, and keeper execution. Hosted releases
   use `TOKENLESS_ARTIFACT_WRAPPING_KEYS`; `TOKENLESS_ARTIFACT_MASTER_KEY` is accepted only during the isolated
   tokenless migration and must be removed after rewrap verification.
...
9. Verify runtime `VERCEL_REGION`, `RAILWAY_REPLICA_REGION`, Railway project/service IDs, Blob privacy, database region,
   backup restore, log delivery, key rotation/recovery, signer address/version pinning, and deletion before customer
   data is admitted. Exercise spend ceilings and confirm every funded signer has only the minimum native balance,
   token balance, and allowance required for its role.

Platform secret stores make operations simpler but allow the application platform to deliver key material to an
authorized runtime. They are not non-exportable HSM custody. The release approval must accept that tradeoff explicitly
and attach content-addressed rotation, recovery, and per-role spend-limit runbooks.
