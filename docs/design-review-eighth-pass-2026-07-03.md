...
1. **6P-5 — 1-hour frontend fee withdrawal vs. multi-day slash latency.** Resolved by freezing `completeFeeWithdrawal()` and the fee-release leg of `completeDeregister()` while the operator has an active challenged payout-root proposal recorded by the oracle, and by softening docs/NatSpec so they no longer claim earned-but-undelivered fees back every fresh offense. `FEE_WITHDRAWAL_DELAY` remains the short fee review window; the 1,000 LREP bond and 14-day stake exit remain the primary backing for fresh offenses that governance resolves after that window. Governance should batch a rejection with the matching slash where appropriate, and parent-epoch rejection batches should explicitly resolve challenged child round snapshots that need the fee freeze released immediately.
...
2. **6P-6 / 7P-7 / governor-lock / mechanism-addendum** — ratify each as an explicit accept-or-fix decision and reconcile the docs; these are the last substantive open items and none requires a large code change.
