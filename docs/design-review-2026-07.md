...
- **L (weak PRNG pairing, historical/superseded):** the pre-remediation snapshot drew reference/peer pairings from a delayed-blockhash seed. The fresh redeploy posture supersedes this with precommitted reveal entropy bound to the closed scoring set, so reviewers should use the current remediation plan rather than this historical pairing model for launch readiness.
...
**F-4 (Low/Informational, historical/superseded for the fresh redeploy). RBTS pairing seed was derived purely from a delayed blockhash and re-rolled on expiry in the reviewed pre-remediation snapshot.** The fresh redeploy posture in `docs/incentives-remediation-plan-2026-07.md` requires reveal-entropy-based pairings instead of a future-blockhash or sequencer non-grinding assumption. Keep this item as historical audit context for why the reveal-entropy plan exists; do not treat the delayed-blockhash model as the launch design.
