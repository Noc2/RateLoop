...
The design's load-bearing property: **no operator path to user funds.** Escrow sits in an immutable adminless fund-holding core; users self-execute; the keeper only calls permissionless functions; payout claims are permissionless (the auto-claimer is a convenience executor, self-claim is the fallback). The separate credential issuer can rotate admission signers and censor future eligibility but cannot touch funds or accepted commits; do not describe the whole system as fully keyless. This gives the best MiCA/PSD2 posture, but it is a posture rather than a conclusion: no on-point precedent exists for fee-charging non-custodial escrow, and the counsel questions remain open. GwG/AMLR: not an obliged entity on the current theory (the AML perimeter tracks the CASP perimeter); keep a decentralization memo current; monitor the 2027+ DeFi reviews. Never hold pooled user balances; B2B prepaid balances sit with a regulated partner. Never build operator-controlled rater key recovery.
...
Wallet addresses, votes (opinions are inherently personal data — CJEU), and identity nullifiers are pseudonymous personal data for the operator. Commitments stay on-chain while content and mappings stay off-chain; erasure means deletion of the mapping after statutory retention (§ 147 AO 8–10y; Art. 18 restriction meanwhile). A one-time vote key is only unlinkable **until claim** when `claim(commitKey, payoutAddress,salt)` exposes the destination; reusing a destination links rounds. The final DPIA may claim cross-round unlinkability only after a user-controlled per-round destination and recovery flow is implemented and tested. DPIA headline scenario remains breach of the vote-key↔rater mapping (a standing deanonymization key for an immutable public record), mitigated by per-rater-key encryption; the operator must not hold a universal decryption or spend key. The DAC7 vault is separate, envelope-encrypted, and deletion-scheduled. As a World ID relying party the company holds no biometric or document data; a non-biometric verification path must reach every earning tier. Privacy notice precedes on-chain commitment; maintain Art. 30 records and documented Chapter V analysis for public-chain replication.
