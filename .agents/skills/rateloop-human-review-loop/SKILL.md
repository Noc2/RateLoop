...
2. Create one stable external opportunity ID for the output. The source payload is the exact reviewer-visible request plus only the context needed to judge it; the suggestion payload is the exact completed candidate response awaiting release. Hash those exact UTF-8 strings during evaluation and reuse the same strings byte-for-byte when requesting review. Never replace them with setup examples, connection checks, tool messages, review-status text, a regenerated answer, or hidden reasoning. Supply only those payloads, their content commitments, declared risk, confidence, completeness, and privacy-safe execution metadata.
