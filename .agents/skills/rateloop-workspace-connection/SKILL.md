...
9. When verification succeeds, report only that the workspace is connected and ready. Do not repeat identifiers or connection material unless the tool explicitly returns a display-safe value intended for the user.
...
- If the workspace tools remain unavailable and the host exposes no native action, say that the connection is not verified in the current task and tell the user once to uninstall every existing RateLoop plugin, including `rateloop` and `rateloop-workspace`. Preserve the original connection intent, do not ask them to paste the link again, and re-check the tools on their next active turn. Never tell them to remove unrelated plugins or guess a reload, restart, toggle, gear, or settings path.
