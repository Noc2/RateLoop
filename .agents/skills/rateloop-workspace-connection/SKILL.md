...
4. The connection message deliberately combines a structured RateLoop plugin mention with an explicit `$rateloop-workspace-connection` invocation so a supported host can present its native install/connect flow in either state. The plugin mention installs RateLoop when it is missing; the explicit skill invocation activates the skill's `rateloop-workspace` dependency when the plugin is already installed. If the host actually presents an install, trust, update, OAuth, **Continue**, restart, or new-task action, state or follow only that exact action while preserving the original intent. Never invent a button, menu, settings path, or host capability. Never rewrite MCP configuration by hand or substitute the public server.
