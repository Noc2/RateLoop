...
- Only if a later active turn still lacks the workspace tools after the first missing-tool check, and the host exposes no native action, say that the connection is not verified in the current task and tell the user once to uninstall every existing RateLoop plugin, including `rateloop` and `rateloop-workspace`. Preserve the original connection intent, do not ask them to paste the link again, and re-check the tools on their next active turn. Never repeat that uninstall recovery for the same intent. Never tell them to remove unrelated plugins or guess a reload, restart, toggle, gear, or settings path.
